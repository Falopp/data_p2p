...
class UnifiedReporter:
    ...
    def __init__(
        self, base_output_dir: str, config: dict, cli_args: argparse.Namespace
    ):
        ...
        self.structure = {
            ...
            "figures": {
                "usd_analysis": os.path.join(
                    base_output_dir, "figures", "usd_analysis"
                ),
                "uyu_analysis": os.path.join(
                    base_output_dir, "figures", "uyu_analysis"
                ),
                "comparative": os.path.join(base_output_dir, "figures", "comparative"),
                "general": os.path.join(base_output_dir, "figures", "general"),
                "counterparty": os.path.join(
                    base_output_dir, "figures", "counterparty"
                ),
    ...
    def _create_directory_structure(self):
        ...
        for path in [
            self.structure["consolidated"],
            self.structure["reports"],
            self.structure["data_exports"],
        ]:
    ...
    def generate_unified_report(
        self, all_period_data: Dict[str, Dict[str, Any]]
    ) -> str:
        """
        ...
        html_path = self._generate_unified_html(
            all_period_data, figure_paths, excel_path
        )

        logger.info(f"Reporte unificado completado: {html_path}")
        return html_path

    def _generate_consolidated_figures(
        self, all_period_data: Dict[str, Dict[str, Any]]
    ) -> Dict[str, List[str]]:
        ...
        general_figures = self._generate_general_plots(
            all_period_data, self.structure["figures"]["general"]
        )
        ...
        counterparty_figures = self._generate_consolidated_counterparty_plots(
            all_period_data, self.structure["figures"]["counterparty"]
        )
        ...
        session_figures = self._generate_consolidated_session_plots(
            all_period_data, self.structure["figures"]["sessions"]
        )
        figure_paths["sessions"].extend(session_figures)

        return figure_paths

    def _consolidate_currency_data(
        self, all_period_data: Dict, currency: str
    ) -> pl.DataFrame:
        ...
        for period_name, period_data in all_period_data.items():
            for status_name, status_data in period_data.items():
                ...
                if not current_df.is_empty() and "fiat_type" in current_df.columns:
                    ...
                    currency_df = current_df.filter(
                        pl.col("fiat_type") == currency
                    ).with_columns(
                        [
    ...
    def _generate_currency_specific_plots(
        self, df: pl.DataFrame, out_dir: str, currency: str, title_suffix: str
    ) -> List[str]:
        ...
        if (
            "Match_time_local" in df_pandas.columns
            and "TotalPrice_num" in df_pandas.columns
        ):
            fig, axes = plt.subplots(2, 2, figsize=(16, 12))
            fig.suptitle(
                f"Evolución Temporal - {title_suffix}", fontsize=16, fontweight="bold"
            )
            ...
            period_volume = (
                df_pandas.groupby(
                    ["period_source", df_pandas["Match_time_local"].dt.to_period("M")]
                )["TotalPrice_num"]
                .sum()
            ...
            status_volume = (
                df_pandas.groupby(
                    ["status_source", df_pandas["Match_time_local"].dt.to_period("M")]
                )["TotalPrice_num"]
                .sum()
                .unstack(level=0, fill_value=0)
            )
            ...
            if "Price_num" in df_pandas.columns:
                axes[1, 1].scatter(
                    df_pandas["Price_num"], df_pandas["TotalPrice_num"], alpha=0.5
                )
        ...
        if "Match_time_local" in df_pandas.columns:
            path_heatmap = self._generate_consolidated_heatmap(
                df_pandas, out_dir, currency, title_suffix
            )
            if path_heatmap:
        ...
        if "payment_method" in df_pandas.columns:
            path_payment = self._generate_payment_analysis(
                df_pandas, out_dir, currency, title_suffix
            )
            if path_payment:
    ...
    def _generate_comparative_plots(
        self, usd_df: pl.DataFrame, uyu_df: pl.DataFrame, out_dir: str
    ) -> List[str]:
        ...
        if all(
            col in usd_pandas.columns for col in ["Match_time_local", "TotalPrice_num"]
        ):
            fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))
            fig.suptitle(
                "Comparación USD vs UYU - Volúmenes Mensuales",
                fontsize=16,
                fontweight="bold",
            )

            # Volumen USD
            usd_monthly = usd_pandas.groupby(
                usd_pandas["Match_time_local"].dt.to_period("M")
            )["TotalPrice_num"].sum()
            usd_monthly.plot(
                ax=ax1, color="green", marker="o", title="Volumen USD/USDT"
            )
            ax1.set_ylabel("Volumen USD")

            # Volumen UYU
            uyu_monthly = uyu_pandas.groupby(
                uyu_pandas["Match_time_local"].dt.to_period("M")
            )["TotalPrice_num"].sum()
        ...
        usd_ops = usd_pandas.groupby(
            usd_pandas["Match_time_local"].dt.to_period("M")
        ).size()
        uyu_ops = uyu_pandas.groupby(
            uyu_pandas["Match_time_local"].dt.to_period("M")
        ).size()

        ax.plot(
            usd_ops.index.astype(str),
            usd_ops.values,
            marker="o",
            label="Operaciones USD/USDT",
            color="green",
        )
        ax.plot(
            uyu_ops.index.astype(str),
            uyu_ops.values,
            marker="s",
            label="Operaciones UYU",
            color="blue",
        )

        ax.set_title(
            "Comparación USD vs UYU - Número de Operaciones",
            fontsize=14,
            fontweight="bold",
        )
    ...
    def _generate_general_plots(self, all_period_data: Dict, out_dir: str) -> List[str]:
        ...
        if "status" in consolidated_pandas.columns:
            ...
            status_period = (
                consolidated_pandas.groupby(["period_source", "status"])
                .size()
            ...
            ax.set_title(
                "Distribución de Estados por Periodo", fontsize=14, fontweight="bold"
            )
            ax.set_ylabel("Número de Operaciones")
        ...
        if (
            "asset_type" in consolidated_pandas.columns
            and "Match_time_local" in consolidated_pandas.columns
        ):
            fig, ax = plt.subplots(figsize=(14, 8))

            asset_monthly = (
                consolidated_pandas.groupby(
                    [
            ...
            ax.set_title(
                "Evolución de Operaciones por Tipo de Activo",
                fontsize=14,
                fontweight="bold",
            )
            ax.set_ylabel("Número de Operaciones")
    ...
    def _generate_consolidated_counterparty_plots(
        self, all_period_data: Dict, out_dir: str
    ) -> List[str]:
        ...
        for period_name, period_data in all_period_data.items():
            for status_name, status_data in period_data.items():
                ...
                for key in counterparty_metric_keys:
                    metric_df_pl = metrics.get(
                        f"counterparty_{key}"
                    )  # Usará 'metrics' de forma segura
                    if (
                        metric_df_pl is not None
                        and isinstance(metric_df_pl, pl.DataFrame)
                        and not metric_df_pl.is_empty()
                    ):
        ...
        if not has_any_cp_data:
            logger.warning(
                "No se encontraron datos de métricas de contrapartes en ningún periodo/estado."
            )
            return []
        ...
        for key, df_list in consolidated_cp_data_pl.items():
            if df_list:
                ...
                try:
                    combined_df_pl = pl.concat(
                        df_list, how="vertical_relaxed"
                    )  # relaxed para manejar pequeñas diferencias de esquema
                    ...
                    if (
                        key == "general_stats"
                        and "Counterparty" in combined_df_pl.columns
                    ):
                        ...
                        logger.warning(
                            f"Métrica '{key}' concatenada. Si hay IDs de contraparte duplicados entre periodos/estados, "
                    ...
                    logger.error(
                        f"Error al concatenar o convertir a Pandas la métrica de contraparte '{key}': {e}"
                    )
        ...
        if not any(not df.empty for df in final_cp_data_pd.values()):
            logger.warning(
                "Todos los DataFrames de métricas de contrapartes consolidados están vacíos después del procesamiento."
            )
            return []
    ...
    def _generate_consolidated_session_plots(
        self, all_period_data: Dict, out_dir: str
    ) -> List[str]:
        ...
        if not has_any_session_data:
            logger.warning(
                "No se encontraron datos de métricas de sesiones en ningún periodo/estado."
            )
            return []
        ...
        for key, df_list in consolidated_session_data_pl.items():
            if df_list:
                try:
                    ...
                    logger.error(
                        f"Error al concatenar o convertir a Pandas la métrica de sesión '{key}': {e}"
                    )
        ...
        if not any(not df.empty for df in final_session_data_pd.values()):
            logger.warning(
                "Todos los DataFrames de métricas de sesiones consolidados están vacíos."
            )
            return []
        ...
        if (
            "session_stats" in final_session_data_pd
            and not final_session_data_pd["session_stats"].empty
        ):
            ...
            if "duration_minutes" in df_plot.columns:
                try:
                    ...
                    sns.histplot(
                        df_plot["duration_minutes"].dropna(), bins=30, kde=True
                    )
                    plt.title("Distribución de Duración de Sesiones (Consolidado)")
                    plt.xlabel("Duración (minutos)")
                    plt.ylabel("Frecuencia")
                    path = os.path.join(
                        out_dir, "session_duration_distribution_consolidated.png"
                    )
                    plt.savefig(path, dpi=300, bbox_inches="tight")
                    plt.close()
                    saved_paths.append(path)
                    ...
                    logger.error(
                        f"Error generando gráfico de duración de sesiones: {e_plot}"
                    )
        ...
        if (
            ...
            if "num_operations" in df_plot.columns:
                try:
                    ...
                    path = os.path.join(
                        out_dir, "session_operations_distribution_consolidated.png"
                    )
                    plt.savefig(path, dpi=300, bbox_inches="tight")
                    plt.close()
                    saved_paths.append(path)
                    ...
                    logger.error(
                        f"Error generando gráfico de operaciones por sesión: {e_plot}"
                    )

        # 3. Distribución de Inicio de Sesiones por Hora del Día
        if (
            "temporal_distribution" in final_session_data_pd
            and not final_session_data_pd["temporal_distribution"].empty
        ):
            df_plot_raw = final_session_data_pd[
                "temporal_distribution"
            ]  # Este ya está agregado por start_hour EN EL ANALIZADOR ORIGINAL
            if (
                "start_hour" in df_plot_raw.columns
                and "num_sessions" in df_plot_raw.columns
            ):
                try:
                    ...
                    df_plot_grouped = df_plot_raw.groupby("start_hour", as_index=False)[
                        "num_sessions"
                    ].sum()

                    hourly_series = df_plot_grouped.set_index("start_hour")[
                        "num_sessions"
                    ].sort_index()
                    hourly_series_reindexed = hourly_series.reindex(
                        range(24), fill_value=0
                    )

                    plt.figure(figsize=(12, 7))
                    sns.barplot(
                        x=hourly_series_reindexed.index,
                        y=hourly_series_reindexed.values,
                        color=sns.color_palette()[2],
                    )
                    plt.title(
                        "Distribución de Inicio de Sesiones por Hora (Consolidado)"
                    )
                    plt.xlabel("Hora de Inicio de Sesión")
                    plt.ylabel("Número de Sesiones")
                    plt.xticks(ticks=range(0, 24, 2))
                    path = os.path.join(
                        out_dir, "session_start_hour_distribution_consolidated.png"
                    )
                    plt.savefig(path, dpi=300, bbox_inches="tight")
                    plt.close()
                    saved_paths.append(path)
                    logger.info(
                        f"Gráfico de inicio de sesiones por hora guardado: {path}"
                    )
                except Exception as e_plot:
                    logger.error(
                        f"Error generando gráfico de inicio de sesiones por hora: {e_plot}"
                    )
        ...
        if (
            ...
            df_plot = final_session_data_pd["session_stats"]
            if (
                "duration_minutes" in df_plot.columns
                and "total_volume" in df_plot.columns
            ):
                try:
                    plt.figure(figsize=(10, 6))
                    sns.scatterplot(
                    ...
                    plt.legend(
                        title="Moneda Dominante / Ops",
                        bbox_to_anchor=(1.05, 1),
                        loc="upper left",
                    )
                    path = os.path.join(
                        out_dir, "session_volume_vs_duration_consolidated.png"
                    )
                    plt.savefig(path, dpi=300, bbox_inches="tight")
                    plt.close()
                    saved_paths.append(path)
                    logger.info(
                        f"Gráfico de volumen vs duración de sesión guardado: {path}"
                    )
                except Exception as e_plot:
                    logger.error(
                        f"Error generando scatter de volumen vs duración de sesión: {e_plot}"
                    )

        logger.info(f"Generados {len(saved_paths)} gráficos de sesiones consolidados.")
        return saved_paths

    def _generate_consolidated_heatmap(
        self, df_pandas: pd.DataFrame, out_dir: str, currency: str, title_suffix: str
    ) -> Union[str, None]:
        ...
        heatmap_data = (
            df_pandas.groupby(["weekday", "hour"]).size().unstack(fill_value=0)
        )
        ...
        ax.set_title(
            f"Actividad por Hora y Día - {title_suffix}", fontsize=14, fontweight="bold"
        )
    ...
    def _generate_payment_analysis(
        self, df_pandas: pd.DataFrame, out_dir: str, currency: str, title_suffix: str
    ) -> str:
        """Genera análisis de métodos de pago."""
        if (
            "payment_method" not in df_pandas.columns
            or "TotalPrice_num" not in df_pandas.columns
        ):
            return None

        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))
        fig.suptitle(
            f"Análisis de Métodos de Pago - {title_suffix}",
            fontsize=16,
            fontweight="bold",
        )
        ...
        payment_volume = (
            df_pandas.groupby("payment_method")["TotalPrice_num"]
            .sum()
    ...
    def _generate_consolidated_excel(self, all_period_data: Dict) -> str:
        ...
        excel_path = os.path.join(
            self.structure["data_exports"], "P2P_Analysis_Consolidated.xlsx"
        )
    ...
    def _create_executive_summary_sheet(
        self, writer: pd.ExcelWriter, all_period_data: Dict
    ):
        ...
        for period_name, period_data in all_period_data.items():
            for status_name, status_data in period_data.items():
                ...
                if not current_df.is_empty():
                    ...
                    if (
                        "fiat_type" in df_pandas.columns
                        and "TotalPrice_num" in df_pandas.columns
                    ):
                        total_volume_usd = df_pandas[df_pandas["fiat_type"] == "USD"][
                            "TotalPrice_num"
                        ].sum()
                    ...
                    if (
                        ...
                        total_volume_uyu = df_pandas[df_pandas["fiat_type"] == "UYU"][
                            "TotalPrice_num"
                        ].sum()

                    unique_counterparties = (
                        df_pandas["Counterparty"].nunique()
                        if "Counterparty" in df_pandas.columns
                        else 0
                    )
            ...
            for period_name, period_data in all_period_data.items():
                for status_name, status_data in period_data.items():
                    ...
                    if (
                        ...
                        and not current_df.is_empty()
                        and "fiat_type" in current_df.columns
                    ):
            ...
            for col in df_for_excel_pd.columns:
                if pd.api.types.is_datetime64_any_dtype(df_for_excel_pd[col]):
                    if df_for_excel_pd[col].dt.tz is not None:
                        logger.info(
                            f"Convirtiendo columna datetime '{col}' a timezone-naive para Excel en hoja {currency}."
                        )
                        df_for_excel_pd[col] = df_for_excel_pd[col].dt.tz_localize(None)

            try:
                df_for_excel_pd.to_excel(
                    writer, sheet_name=f"Datos_{currency}", index=False
                )
                logger.info(
                    f"Hoja 'Datos_{currency}' creada en Excel con {df_for_excel_pd.shape[0]} filas."
                )
            except Exception as e_excel:
                logger.error(
                    f"Error al escribir la hoja 'Datos_{currency}' en Excel: {e_excel}"
                )

    def _create_period_metrics_sheets(
        self, writer: pd.ExcelWriter, all_period_data: Dict
    ):
    ...
    def _generate_unified_html(
        self, all_period_data: Dict, figure_paths: Dict, excel_path: str
    ) -> str:
        ...
        for period_data in all_period_data.values():
            for status_data in period_data.values():
                ...
                if not current_df.is_empty():
                    ...
                    if (
                        "fiat_type" in current_df.columns
                        and "TotalPrice_num" in current_df.columns
                    ):
                    ...
                    if "Counterparty" in current_df.columns:
                        ...
                        unique_counterparties.update(
                            [cp for cp in counterparties if cp is not None]
                        )
