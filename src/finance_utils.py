...
def calculate_rolling_pnl(daily_returns: pl.Series, window_size: int, initial_investment: float = 1000.0) -> pl.Series:
    """Calcula el P&L acumulado rodante sobre una ventana dada."""
    if daily_returns.is_empty() or daily_returns.len() < window_size:
        logger.warning(f"No hay suficientes datos para calcular P&L rodante con ventana {window_size}")
        return pl.Series([], dtype=pl.Float64)
    ...
    sum_log_rolling = log_one_plus_returns.rolling_sum(window_size=window_size, min_periods=window_size)
    ...
    return portfolio_value_rolling.fill_null(strategy='forward') # Llenar nulos iniciales

def calculate_sharpe_ratio(daily_returns: pl.Series, risk_free_rate_annual: float = 0.0, periods_per_year: int = 252) -> float | None:
    """Calcula el Ratio de Sharpe anualizado a partir de retornos diarios."""
    if daily_returns.is_empty() or daily_returns.std() == 0 or daily_returns.std() is None:
        logger.warning("No se puede calcular el Ratio de Sharpe: retornos vacíos o desviación estándar cero/nula.")
        return None
    ...
    if std_dev_excess_return == 0 or std_dev_excess_return is None:
        logger.warning("No se puede calcular el Ratio de Sharpe: desviación estándar de retornos excedentes es cero/nula.")
        return None
...
if __name__ == '__main__':
    ...
    dates = pl.date_range(pl.Date(2023,1,1), pl.Date(2023,1,10), "1d", eager=True).alias("date")
    ...
    window = 3
    df_prices = df_prices.with_columns(
        calculate_rolling_pnl(pl.col("daily_returns"), window_size=window).alias(f"rolling_pnl_{window}d")
    )
