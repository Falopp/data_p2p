...
def _get_adaptive_temporal_settings(title_suffix: str = "") -> dict:
    """Detecta si estamos en modo filtro de mes y ajusta las configuraciones temporales.

    Args:
        title_suffix: Sufijo del título que puede contener información de filtros

    Returns:
        Diccionario con configuraciones adaptadas para ventanas, agregaciones, etc.
    """
    # Detectar si hay filtro de mes en el title_suffix
    is_month_filter = "mes " in title_suffix.lower() or "month " in title_suffix.lower()

    if is_month_filter:
        # Configuraciones para datos de un solo mes (temporalidades más granulares)
        return {
            "rolling_window_short": 3,  # 3 días en lugar de 7
            "rolling_window_long": 7,  # 7 días en lugar de 30
            "monthly_rolling_window": 1,  # 1 día en lugar de 3 meses
            "time_format": "%m-%d %H:%M",  # Formato día-hora sin año
            "time_locator_bins": 10,  # Menos bins en eje temporal
            "aggregation_level": "daily",  # Agregación diaria en lugar de mensual
            "heatmap_annotation": True,  # Mostrar anotaciones en heatmaps
            "price_bins": 20,  # Menos bins para histogramas
            "volume_threshold_factor": 0.5,  # Factor más bajo para filtros de volumen
        }
    else:
        # Configuraciones para datos completos (temporalidades estándar)
        return {
            "rolling_window_short": 7,  # 7 días estándar
            "rolling_window_long": 30,  # 30 días estándar
            "monthly_rolling_window": 3,  # 3 meses estándar
            "time_format": "%Y-%m-%d",  # Formato completo con año
            "time_locator_bins": 15,  # Más bins en eje temporal
            "aggregation_level": "monthly",  # Agregación mensual estándar
            "heatmap_annotation": False,  # Sin anotaciones para evitar saturación
            "price_bins": 50,  # Más bins para histogramas
            "volume_threshold_factor": 1.0,  # Factor estándar
        }
...
def plot_hourly(
    ...
    hourly_counts_reindexed = hourly_counts.reindex(
    ...
    if (
        pd.notna(mean_operations) and mean_operations > 0
    ):  # Solo si la media es válida y positiva
        ax.axhline(
            mean_operations,
            color="red",
            linestyle="--",
            linewidth=1.5,
            label=f"Promedio: {mean_operations:.2f} ops/hr",
        )
...
def plot_monthly(
    monthly_fiat: pd.DataFrame,
    out_dir: str,
    title_suffix: str = "",
    file_identifier: str = "_general",
) -> list[str]:
    """Genera gráficos de evolución temporal (mensual o diaria según contexto) con configuraciones adaptivas."""
    saved_paths = []
    if monthly_fiat.empty:
        logger.info(
            f"No hay datos para el gráfico de evolución temporal{title_suffix}."
        )
        return saved_paths

    # Obtener configuraciones adaptivas basadas en el contexto (filtro de mes o datos completos)
    adaptive_settings = _get_adaptive_temporal_settings(title_suffix)
    aggregation_level = adaptive_settings["aggregation_level"]
    monthly_rolling_window = adaptive_settings["monthly_rolling_window"]
    ...
    if fiat_type_col_internal in monthly_fiat.columns:
        ...
        isinstance(monthly_fiat.index, pd.MultiIndex)
        and fiat_type_col_internal in monthly_fiat.index.names
    ):
        unique_fiats = monthly_fiat.index.get_level_values(
    ...
    for current_fiat in unique_fiats:
        if current_fiat is not None:
            if fiat_column_for_grouping:
                df_plot_full = monthly_fiat[
                    monthly_fiat[fiat_column_for_grouping] == current_fiat
                ]
            elif isinstance(monthly_fiat.index, pd.MultiIndex):
                df_plot_full = monthly_fiat.xs(
                    current_fiat, level=fiat_type_col_internal
                )
            else:
                df_plot_full = monthly_fiat[monthly_fiat.index == current_fiat]
            current_fiat_label_for_title = str(current_fiat).upper()
            plot_title_fiat_part = f" en {current_fiat_label_for_title}"
            current_fiat_label_for_file = str(current_fiat).lower()
        else:
            df_plot_full = monthly_fiat
            current_fiat_label_for_title = "Todas las Monedas"
            plot_title_fiat_part = ""
            current_fiat_label_for_file = "all_fiats"

        if df_plot_full.empty:
            logger.info(
                f"No hay datos para el fiat '{current_fiat}' en gráfico temporal{title_suffix}."
            )
            continue
        ...
        # Si hay datos con columna de tiempo, intentar crear plot_date
        if year_month_col_internal in df_plot_full.columns:
            df_plot_full = df_plot_full.copy()
            try:
                # Para modo mes (agregación diaria), usar date parsing directo
                if aggregation_level == "daily":
                    # Asumir que tenemos datos diarios en formato apropiado
                    df_plot_full["plot_date"] = pd.to_datetime(
                        df_plot_full[year_month_col_internal], errors="coerce"
                    )
                else:
                    # Modo mensual estándar
                    df_plot_full["plot_date"] = pd.to_datetime(
                        df_plot_full[year_month_col_internal] + "-01",
                        format="%Y-%m-%d",
                        errors="coerce",
                    )
                df_plot_full = df_plot_full.dropna(subset=["plot_date"])
                df_plot_full = df_plot_full.sort_values("plot_date")
            except Exception as e_date_parse:
                logger.warning(
                    f"Error al parsear fechas en gráfico temporal: {e_date_parse}. Usando índice original."
                )
                df_plot_full["plot_date"] = df_plot_full.index

        # Filtrar columnas numéricas para graficar
        potential_value_cols = [
            col
            for col in df_plot_full.columns
            if col not in [year_month_col_internal, fiat_type_col_internal, "plot_date"]
        ]
        potential_value_cols = [
            col
            for col in potential_value_cols
            if pd.api.types.is_numeric_dtype(df_plot_full[col])
        ]

        if not potential_value_cols:
            logger.warning(
                f"No se encontraron columnas numéricas para graficar en evolución temporal para {current_fiat}{title_suffix}."
            )
            plt.close()
            continue
        ...
        for value_col in potential_value_cols:
            if value_col in df_plot_full and pd.api.types.is_numeric_dtype(
                df_plot_full[value_col]
            ):
                series_to_plot = (
                    df_plot_full.set_index("plot_date")[value_col]
                    if "plot_date" in df_plot_full
                    else df_plot_full.set_index(year_month_col_internal)[value_col]
                )
                ...
                # Media móvil adaptiva
                if len(series_to_plot) >= monthly_rolling_window:
                    rolling_mean_adaptive = series_to_plot.rolling(
                        window=monthly_rolling_window, center=True, min_periods=1
                    ).mean()

                    if aggregation_level == "daily":
                        rolling_label = (
                            f"{value_col} (Media Móvil {monthly_rolling_window}D)"
                        )
                    else:
                        rolling_label = (
                            f"{value_col} (Media Móvil {monthly_rolling_window}M)"
                        )

                    plt.plot(
                        rolling_mean_adaptive.index,
                        rolling_mean_adaptive.values,
                        linestyle="--",
                        color=line_color,
                        alpha=0.8,
                        label=rolling_label,
        ...
        # Adaptar títulos según el nivel de agregación
        if aggregation_level == "daily":
            main_chart_title = (
                f"Evolución Diaria del Volumen P2P{plot_title_fiat_part}{title_suffix}"
            )
            fig_text_explanation_temporal = (
                f"Este gráfico muestra la evolución diaria del volumen total transaccionado en {current_fiat_label_for_title}. "
                f"Las líneas de colores representan diferentes tipos de operación (ej. Compra, Venta).\\n"
                f"Las líneas discontinuas muestran una media móvil de {monthly_rolling_window} días para suavizar las fluctuaciones y destacar tendencias."
            )
            time_axis_label = "Fecha (Día)"
        else:
            main_chart_title = (
                f"Evolución Mensual del Volumen P2P{plot_title_fiat_part}{title_suffix}"
            )
            fig_text_explanation_temporal = (
                f"Este gráfico muestra la evolución mensual del volumen total transaccionado en {current_fiat_label_for_title}. "
                f"Las líneas de colores representan diferentes tipos de operación (ej. Compra, Venta).\\n"
                f"Las líneas discontinuas muestran una media móvil de {monthly_rolling_window} meses para suavizar las fluctuaciones y destacar tendencias."
            )
            time_axis_label = "Mes (Año-Mes)"

        plt.suptitle(main_chart_title, fontsize=16, y=1.03)
        ax.set_title(fig_text_explanation_temporal, fontsize=10, pad=20)

        plt.xlabel(time_axis_label, fontsize=12)
        plt.ylabel(
            f"Volumen Total Transaccionado ({current_fiat_label_for_title})",
            fontsize=12,
        )

        plt.xticks(rotation=45, ha="right")

        # Usar formato de tiempo adaptivo
        if aggregation_level == "daily":
        ...
        plt.legend(title="Tipo de Operación / Media Móvil", fontsize=9)
        plt.grid(True, linestyle="--", alpha=0.7)

        ax.yaxis.set_major_formatter(
            mticker.FuncFormatter(
                lambda x, p: utils.format_large_number(x) if x >= 1000 else f"{x:,.0f}"
            )
        )

        plt.tight_layout(rect=[0, 0, 1, 0.96])

        # Adaptar nombre de archivo
        if aggregation_level == "daily":
            file_name_temporal = (
                f"daily_evolution_{current_fiat_label_for_file}{file_identifier}.png"
            )
        else:
            file_name_temporal = (
                f"monthly_evolution_{current_fiat_label_for_file}{file_identifier}.png"
            )

        file_path = os.path.join(out_dir, file_name_temporal)
        try:
            plt.savefig(file_path, dpi=300)
            saved_paths.append(file_path)
            logger.info(
                f"Gráfico de evolución temporal guardado (agregación {aggregation_level}, ventana {monthly_rolling_window}): {file_path}"
            )
        except Exception as e:
            logger.error(
                f"Error al guardar el gráfico de evolución temporal {file_path}: {e}"
            )
        finally:
            plt.close()
...
def plot_pie(
    ...
    if (
        df_counts.empty
        or column_name not in df_counts.columns
        or df_counts[column_name].sum() == 0
    ):
    ...
    if not slices:  # Todos los slices eran demasiado pequeños
        logger.info(
            f"Todos los slices para '{title}' son menores que {min_percentage_for_slice}%. No se generará gráfico de torta.{title_suffix}"
        )
        return None
    ...
    plt.tight_layout(
        rect=[0, 0, 0.85, 0.95]
    )  # Ajustar para dar espacio a la leyenda y suptitle
...
def plot_price_distribution(
    ...
    if df_completed.empty:
        logger.info(
            f"No hay datos completados para graficar distribución de precios{title_suffix}."
        )
        return saved_paths
    ...
    if not all(col in df_completed.columns for col in required_cols):
        ...
        logger.warning(
            f"Faltan columnas para graficar distribución de precios ({', '.join(missing)}){title_suffix}."
        )
        return saved_paths

    # Iterar por cada par de Asset/Fiat
    for (asset, fiat), group_data in df_completed.groupby(
        ...
        try:
            if group_data.empty or group_data[price_num_col].isna().all():
                logger.info(
                    f"Omitiendo gráfico de distribución de precios para {asset}/{fiat} (datos vacíos o NaN){title_suffix}."
                )
                continue
            ...
            group_data_cleaned[price_num_col] = pd.to_numeric(
                group_data_cleaned[price_num_col], errors="coerce"
            )
            group_data_cleaned = group_data_cleaned.dropna(subset=[price_num_col])

            if group_data_cleaned.empty:
                logger.info(
                    f"Omitiendo gráfico de distribución de precios para {asset}/{fiat} (datos vacíos después de limpiar NaNs en precio){title_suffix}."
                )
                continue
            ...
            axes[0].set_title(
                sub_title_text, fontsize=10, pad=10
            )  # Título para el primer subplot (actúa como subtítulo general)

            # Histograma en el primer subplot (axes[0])
            sns.histplot(
                data=group_data_cleaned,
                x=price_num_col,
            ...
            for order_t, color in zip(
                pd.unique(group_data_cleaned[order_type_col_internal]),
                sns.color_palette(
                    "muted",
                    n_colors=group_data_cleaned[order_type_col_internal].nunique(),
                ),
            ):
                subset = group_data_cleaned[
                    group_data_cleaned[order_type_col_internal] == order_t
                ]
                if not subset.empty:
                    mean_price = subset[price_num_col].mean()
                    median_price = subset[price_num_col].median()
                    if pd.notna(mean_price):
                        line_mean = axes[0].axvline(
                            mean_price,
                            color=color,
                            linestyle="--",
                            linewidth=1.2,
                            label=f"Media {order_t}: {mean_price:.2f}",
                        )
                        if f"Media {order_t}" not in legend_labels_hist:
                            legend_handles_hist.append(line_mean)
                            legend_labels_hist.append(
                                f"Media {order_t}: {utils.format_large_number(mean_price, precision=2)}"
                            )
                    if pd.notna(median_price):
                        line_median = axes[0].axvline(
                            median_price,
                            color=color,
                            linestyle=":",
                            linewidth=1.2,
                            label=f"Mediana {order_t}: {median_price:.2f}",
                        )
                        if f"Mediana {order_t}" not in legend_labels_hist:
                            legend_handles_hist.append(line_median)
                            legend_labels_hist.append(
                                f"Mediana {order_t}: {utils.format_large_number(median_price, precision=2)}"
                            )

            axes[0].legend(
                handles=legend_handles_hist,
                labels=legend_labels_hist,
                title=f"Tipo Orden / Estadísticas",
                fontsize=9,
            )
            ...
            plt.tight_layout(
                rect=[0, 0.03, 1, 0.93]
            )  # Ajustar rect para suptitle y subtítulo
            plt.savefig(file_path, dpi=300)
            saved_paths.append(file_path)

        except (Exception, KeyboardInterrupt) as e:
            logger.error(
                f"Fallo al generar/guardar gráfico {file_path} para {asset}/{fiat}{title_suffix} debido a {type(e).__name__}: {e}. Creando imagen placeholder."
            )
            if isinstance(e, KeyboardInterrupt):
                logger.warning(
                    f"Generación de gráfico para {file_path} interrumpida manualmente."
                )
            try:
                ...
                error_ax.axis("off")
                plt.tight_layout()
                error_fig.savefig(file_path, dpi=100)
...
def plot_price_over_time(
    ...
    """Genera gráficos de evolución de precios con configuraciones adaptivas para filtros de mes."""
    saved_paths = []
    if df_completed.empty:
        logger.info(
            f"No hay datos completados para graficar Precio a lo largo del Tiempo{title_suffix}."
        )
        return saved_paths
    ...
    # Usar configuraciones adaptivas para las ventanas de medias móviles
    rolling_window_short = adaptive_settings["rolling_window_short"]
    rolling_window_long = adaptive_settings["rolling_window_long"]
    time_format = adaptive_settings["time_format"]
    time_locator_bins = adaptive_settings["time_locator_bins"]
    ...
    if not all(col in df_completed.columns for col in required_cols):
        missing = [col for col in required_cols if col not in df_completed.columns]
        logger.warning(
            f"Faltan columnas ({missing}) para Precio a lo largo del Tiempo{title_suffix}."
        )
        return saved_paths
    ...
    try:
        if not pd.api.types.is_datetime64_any_dtype(df_plot_base[match_time_local_col]):
            df_plot_base[match_time_local_col] = pd.to_datetime(
                df_plot_base[match_time_local_col], errors="coerce"
            )
        df_plot_base[price_num_col] = pd.to_numeric(
            df_plot_base[price_num_col], errors="coerce"
        )
        df_plot_base.dropna(subset=[match_time_local_col, price_num_col], inplace=True)
    except Exception as e:
        logger.error(
            f"Error en la conversión de tipos para Precio a lo largo del Tiempo: {e}{title_suffix}."
        )
        return saved_paths

    if df_plot_base.empty:
        logger.info(
            f"Datos vacíos después de conversión/NaN drop para Precio a lo largo del Tiempo{title_suffix}."
        )
        return saved_paths

    # Iterar por cada par Asset/Fiat
    for (asset_val, fiat_val), group_data_asset_fiat in df_plot_base.groupby(
        [asset_type_col_internal, fiat_type_col_internal]
    ):
        if group_data_asset_fiat.empty:
            logger.info(
                f"Datos vacíos para el par {asset_val}/{fiat_val} en Precio a lo largo del Tiempo{title_suffix}."
            )
            continue
        ...
        palette = sns.color_palette(
            "husl", n_colors=group_data_sorted[order_type_col_internal].nunique() * 3
        )  # Colores para datos y medias
        color_idx = 0

        # Iterar por cada tipo de orden (BUY/SELL)
        for order_val, order_group in group_data_sorted.groupby(
            order_type_col_internal
        ):
            if order_group.empty or order_group[price_num_col].isna().all():
                logger.info(
                    f"Datos vacíos o todos NaN para {order_val} en {asset_val}/{fiat_val} en Precio a lo largo del Tiempo{title_suffix}."
                )
                continue
            ...
            ax.plot(
                order_group[match_time_local_col],
                order_group[price_num_col],
                marker=".",
                linestyle="-",
                alpha=0.4,
                label=f"Precio {order_val}",
                color=base_color,
            )

            # Media Móvil Corta (adaptada)
            if len(order_group) >= rolling_window_short:
                short_rolling_mean = (
                    order_group[price_num_col]
                    .rolling(window=rolling_window_short, center=True, min_periods=1)
                    .mean()
                )
                ax.plot(
                    order_group[match_time_local_col],
                    short_rolling_mean,
                    linestyle="--",
                    label=f"Media Móvil {rolling_window_short}P {order_val}",
                    color=palette[color_idx * 3 + 1],
                    linewidth=1.5,
                )

            # Media Móvil Larga (adaptada)
            if len(order_group) >= rolling_window_long:
                long_rolling_mean = (
                    order_group[price_num_col]
                    .rolling(window=rolling_window_long, center=True, min_periods=1)
                    .mean()
                )
                ax.plot(
                    order_group[match_time_local_col],
                    long_rolling_mean,
                    linestyle=":",
                    label=f"Media Móvil {rolling_window_long}P {order_val}",
                    color=palette[color_idx * 3 + 2],
                    linewidth=1.8,
                )
            color_idx += 1

        main_title = f"Evolución del Precio para {asset_val}/{fiat_val}{title_suffix}"
        # Adaptar el subtítulo basado en las ventanas adaptivas
        sub_title_text = f"Muestra el precio de las transacciones ({asset_val} en {fiat_val}) a lo largo del tiempo, con medias móviles de {rolling_window_short} y {rolling_window_long} periodos (adaptado para granularidad temporal)."
        ...
        # Usar formato de tiempo adaptivo
        ax.xaxis.set_major_formatter(plt.matplotlib.dates.DateFormatter(time_format))
        plt.xticks(rotation=45, ha="right")

        # Ajustar la densidad de los ticks del eje X si hay muchos datos
        num_data_points = len(
            group_data_sorted[match_time_local_col].unique()
        )  # Estimación del número de ticks necesarios
        if num_data_points > 20:  # Umbral ajustable
            ax.xaxis.set_major_locator(
                mticker.MaxNLocator(nbins=time_locator_bins, prune="both")
        ...
        try:
            ...
            saved_paths.append(file_path)
            logger.info(
                f"Gráfico de precio temporal guardado (ventanas adaptivas {rolling_window_short}/{rolling_window_long}P): {file_path}"
            )
        except Exception as e:
            logger.error(
                f"Error al guardar el gráfico Precio a lo largo del Tiempo {file_path}: {e}"
            )
        finally:
            plt.close(fig)
...
def plot_volume_over_time(
    ...
    """Genera gráficos de evolución de volumen con configuraciones adaptivas para filtros de mes."""
    saved_paths = []
    if df_completed.empty:
        logger.info(
            f"No hay datos completados para graficar Volumen a lo largo del Tiempo{title_suffix}."
        )
        return saved_paths
    ...
    if not all(col in df_completed.columns for col in required_cols_base):
        ...
        logger.warning(
            f"Faltan columnas base ({missing}) para Volumen a lo largo del Tiempo{title_suffix}."
        )
        return saved_paths
    ...
    if quantity_num_col in df_completed.columns:
        volume_cols_to_plot_map[quantity_num_col] = (
            "Volumen de Activo",
            asset_type_col_internal,
        )
    if total_price_num_col in df_completed.columns:
        volume_cols_to_plot_map[total_price_num_col] = (
            "Volumen en Fiat",
            fiat_type_col_internal,
        )
    ...
    if not volume_cols_to_plot_map:
        logger.warning(
            f"No se encontraron columnas de volumen ({quantity_num_col} o {total_price_num_col}) para graficar{title_suffix}."
        )
        return saved_paths
    ...
    try:
        ...
        for vol_col_name in volume_cols_to_plot_map.keys():
            if vol_col_name in df_plot_base.columns:
                df_plot_base[vol_col_name] = pd.to_numeric(
                    df_plot_base[vol_col_name], errors="coerce"
                )
        df_plot_base.dropna(
            subset=[match_time_local_col] + list(volume_cols_to_plot_map.keys()),
            how="any",
            inplace=True,
        )
    except Exception as e:
        logger.error(
            f"Error en la conversión de tipos para Volumen a lo largo del Tiempo: {e}{title_suffix}."
        )
        return saved_paths

    if df_plot_base.empty:
        logger.info(
            f"Datos vacíos después de conversión/NaN drop para Volumen a lo largo del Tiempo{title_suffix}."
        )
        return saved_paths
    ...
    for (asset_val, fiat_val), group_data_asset_fiat in df_plot_base.groupby(
        ...
        if group_data_asset_fiat.empty:
            logger.info(
                f"Datos vacíos para el par {asset_val}/{fiat_val} en Volumen a lo largo del Tiempo{title_suffix}."
            )
            continue
        ...
        for vol_col, (
            vol_label_base,
            unit_col_name_for_label,
        ) in volume_cols_to_plot_map.items():
            if (
                group_data_sorted[vol_col].isna().all()
                or (group_data_sorted[vol_col] == 0).all()
            ):
                logger.info(
                    f"Volumen en '{vol_col}' es todo NaN o cero para {asset_val}/{fiat_val}. Omitiendo gráfico.{title_suffix}"
                )
                continue
            ...
            label_unit = (
                asset_val
                if unit_col_name_for_label == asset_type_col_internal
                else fiat_val
            )
            ...
            # Adaptar el subtítulo basado en las ventanas adaptivas
            current_plot_subtitle = f"Muestra el {vol_label_base.lower()} transaccionado ({asset_val} en {fiat_val}) a lo largo del tiempo, con medias móviles de {rolling_window_short} y {rolling_window_long} periodos (adaptado para granularidad temporal)."

            # Graficar la serie de volumen principal
            ax.plot(
                group_data_sorted[match_time_local_col],
                group_data_sorted[vol_col],
                marker=".",
                linestyle="-",
                alpha=0.5,
                label=f"{vol_label_base}",
            ...
            if len(group_data_sorted) >= rolling_window_short:
                ...
                ax.plot(
                    ...
                    short_rolling_mean,
                    linestyle="--",
                    label=f"Media Móvil {rolling_window_short}P",
            ...
            if len(group_data_sorted) >= rolling_window_long:
                ...
                ax.plot(
                    ...
                    long_rolling_mean,
                    linestyle=":",
                    label=f"Media Móvil {rolling_window_long}P",
            ...
            # Usar formato de tiempo adaptivo
            ax.xaxis.set_major_formatter(
                plt.matplotlib.dates.DateFormatter(time_format)
            )
            plt.xticks(rotation=45, ha="right")
            ...
            vol_type_suffix_filename = (
                "fiat_value" if vol_col == total_price_num_col else "asset_quantity"
            )
            ...
            try:
                ...
                saved_paths.append(file_path)
                logger.info(
                    f"Gráfico de volumen temporal guardado (ventanas adaptivas {rolling_window_short}/{rolling_window_long}P): {file_path}"
                )
            except Exception as e:
                logger.error(
                    f"Error al guardar el gráfico Volumen a lo largo del Tiempo {file_path}: {e}"
                )
            finally:
                plt.close(fig)
...
def plot_price_vs_payment_method(
    ...
    if df_completed.empty:
        logger.info(
            f"No hay datos completados para graficar Precio vs Método de Pago{title_suffix}."
        )
        return saved_paths
    ...
    if not all(col in df_completed.columns for col in required_cols_in_df):
        missing = [
            col for col in required_cols_in_df if col not in df_completed.columns
        ]
        logger.warning(
            f"Faltan columnas ({missing}) para Precio vs Método de Pago{title_suffix}."
        )
        return saved_paths

    df_plot_base = df_completed.copy()
    df_plot_base[payment_method_col_internal] = df_plot_base[
        payment_method_col_internal
    ].fillna("Desconocido")
    df_plot_base[price_num_col] = pd.to_numeric(
        df_plot_base[price_num_col], errors="coerce"
    ...
    df_plot_base.dropna(
        subset=[price_num_col, payment_method_col_internal], inplace=True
    )

    if df_plot_base.empty:
        logger.info(
            f"Datos vacíos después de limpieza para Precio vs Método de Pago{title_suffix}."
        )
        return saved_paths
    ...
    for (asset_val, fiat_val), group_data_asset_fiat in df_plot_base.groupby(
        ...
        if (
            group_data_asset_fiat.empty
            or group_data_asset_fiat[payment_method_col_internal].nunique() == 0
            or group_data_asset_fiat[price_num_col].isna().all()
        ):
            logger.info(
                f"Omitiendo {asset_val}/{fiat_val} por datos insuficientes para Precio vs Método de Pago{title_suffix}."
            )
            continue

        # Limitar al Top N métodos de pago por frecuencia para este par Asset/Fiat
        payment_methods_to_show = (
            group_data_asset_fiat[payment_method_col_internal]
            .value_counts()
            .nlargest(top_n_methods)
            .index
        )
        plot_data_final = group_data_asset_fiat[
            group_data_asset_fiat[payment_method_col_internal].isin(
                payment_methods_to_show
            )
        ]
        ...
        if plot_data_final.empty or effective_pm_count == 0:
            logger.info(
                f"Omitiendo {asset_val}/{fiat_val} (después de Top N) por datos insuficientes para Precio vs Método de Pago{title_suffix}."
            )
            continue
        ...
        try:
            ...
            logger.error(
                f"Error al guardar el gráfico Precio vs Método de Pago {file_path}: {e}"
            )
        finally:
            plt.close(fig)
...
def plot_activity_heatmap(
    ...
    """Genera heatmap de actividad con configuraciones adaptivas para filtros de mes."""
    logger.info(
        f"Generando Heatmap de Actividad (valor: {value_col}, agg: {agg_func}){title_suffix}."
    )

    if df_all_statuses.empty:
        logger.warning(
            f"DataFrame de entrada vacío para Heatmap de Actividad.{title_suffix}"
        )
        return None

    # Obtener configuraciones adaptivas basadas en el contexto (filtro de mes o datos completos)
    adaptive_settings = _get_adaptive_temporal_settings(title_suffix)
    show_annotations = adaptive_settings["heatmap_annotation"]
    ...
    if match_time_local_col not in df_plot.columns:
        logger.warning(
            f"Columna de tiempo '{match_time_local_col}' no encontrada para Heatmap.{title_suffix}"
        )
        return None
    if not pd.api.types.is_datetime64_any_dtype(df_plot[match_time_local_col]):
        try:
            df_plot[match_time_local_col] = pd.to_datetime(
                df_plot[match_time_local_col], errors="coerce"
            )
        except Exception as e_time_conv:
            logger.error(
                f"Error convirtiendo '{match_time_local_col}' a datetime: {e_time_conv}.{title_suffix}"
            )
            return None
    df_plot.dropna(
        subset=[match_time_local_col], inplace=True
    )  # Eliminar filas donde la conversión de tiempo falló
    if df_plot.empty:
        logger.warning(
            f"DataFrame vacío después de procesar columna de tiempo para Heatmap.{title_suffix}"
        )
        return None
    ...
    if actual_agg_func == "count":
        ...
        if value_col not in df_plot.columns:
            logger.warning(
                f"Columna de valor '{value_col}' para 'sum' no encontrada en Heatmap.{title_suffix}"
            )
            return None
        if not pd.api.types.is_numeric_dtype(df_plot[value_col]):
            try:
                ...
                df_plot.dropna(
                    subset=[value_col], inplace=True
                )  # Eliminar filas donde la conversión de valor falló
            except Exception as e_val_conv:
                logger.error(
                    f"Error convirtiendo columna de valor '{value_col}' a numérico: {e_val_conv}.{title_suffix}"
                )
                return None
        if df_plot.empty:
            logger.warning(
                f"DataFrame vacío después de procesar columna de valor '{value_col}' para Heatmap.{title_suffix}"
            )
            return None
    else:
        logger.error(
            f"Función de agregación '{agg_func}' no soportada para Heatmap. Usar 'count' o 'sum'."
        )
        return None
    ...
    try:
        ...
        df_plot[day_of_week_col] = df_plot[
            match_time_local_col
        ].dt.day_name()  # Nombres: 'Monday', 'Tuesday', etc.
    except Exception as e_dt_extract:
        logger.error(
            f"Error extrayendo hora/día de '{match_time_local_col}': {e_dt_extract}.{title_suffix}"
        )
        return None
    ...
    df_plot[day_of_week_col] = pd.Categorical(
    ...
    if (
        ...
        or (
            actual_agg_func == "sum"
            and activity_matrix.sum().sum() == 0
            and df_plot[value_col].sum() == 0
        )
        or (
            actual_agg_func == "count"
            and activity_matrix.sum().sum() == 0
            and len(df_plot) == 0
        )
    ):
    ...
    fmt_str = (
        "d" if actual_agg_func == "count" else ",.0f"
    )  # Formato para anotaciones (enteros para count, float para sum)
    cbar_label_val = value_col.replace("_", " ").title() if value_col else "Conteo"
    cbar_label = (
        f"{agg_func.capitalize()} de {cbar_label_val}"
        if actual_agg_func == "sum"
        else "Cantidad de Operaciones"
    )

    # Usar configuración adaptiva para anotaciones
    sns.heatmap(
        activity_matrix,
    ...
    context_info = (
        " (granularidad adaptada para análisis de mes)" if show_annotations else ""
    )
    main_title = f"Heatmap de Actividad de Operaciones por Hora y Día ({agg_func.capitalize()} de {cbar_label_val}){title_suffix}{context_info}"
    ...
    file_path = os.path.join(
        out_dir,
        f"activity_heatmap_{clean_value_col_name}_{clean_agg_func_name}{file_identifier}.png",
    )

    try:
        fig.savefig(file_path, dpi=300)
        logger.info(
            f"Heatmap de actividad guardado (anotaciones: {show_annotations}): {file_path}"
        )
        return file_path
    except Exception as e_save:
        logger.error(f"Error al guardar heatmap de actividad {file_path}: {e_save}")
        return None
    finally:
        plt.close(fig)
...
def plot_fees_analysis(
    ...
    index_level_name: (
        str | None
    ) = None,  # Nombre del nivel del índice a usar para el eje X si es un MultiIndex
) -> str | None:
    ...
    if plot_col not in fees_stats_df.columns:
        ...
        if fees_stats_df.shape[1] == 1 and (
            plot_col == fees_stats_df.iloc[:, 0].name or plot_col == "value"
        ):  # Caso común si era una Serie
            df_plot = fees_stats_df.copy()
            df_plot.rename(columns={df_plot.columns[0]: "data_value"}, inplace=True)
            plot_col = "data_value"
        else:
            logger.warning(
                f"Columna de valor de comisión '{plot_col}' no encontrada en fees_stats_df para graficar{title_suffix}."
            )
            return None
    else:
    ...
    if index_level_name and isinstance(fees_stats_df.index, pd.MultiIndex):
        if index_level_name in fees_stats_df.index.names:
            ...
            logger.warning(
                f"Nivel de índice '{index_level_name}' no encontrado. Usando el primer nivel como fallback.{title_suffix}"
            )
            x_labels = fees_stats_df.index.get_level_values(0)
            x_label_text = (
                fees_stats_df.index.names[0].replace("_", " ").title()
                if fees_stats_df.index.names[0]
                else "Categoría"
            )
    else:
        x_labels = df_plot.index
        x_label_text = (
            df_plot.index.name if df_plot.index.name is not None else "Categoría"
        )
    ...
    if not pd.api.types.is_numeric_dtype(df_plot[plot_col]):
        try:
            ...
            logger.error(
                f"Error convirtiendo columna de comisiones '{plot_col}' a numérica: {e_conv_fees}{title_suffix}."
            )
            return None

    if df_plot.empty or df_plot[plot_col].sum() == 0:
        logger.info(
            f"No hay comisiones significativas (>0) en '{plot_col}' para graficar{title_suffix}."
        )
        return None
    ...
    x_labels_sorted = x_labels[
        df_plot_sorted.index
    ]  # Reordenar etiquetas del eje X según el sort de df_plot

    fig, ax = plt.subplots(figsize=(12, 8))

    bars = ax.bar(
        x_labels_sorted.astype(str),
        df_plot_sorted[plot_col],
        width=0.8,
        color=sns.color_palette("viridis", len(df_plot_sorted)),
    )
    ...
    ax.set_ylabel(
        f'Monto Total de Comisión ({plot_col.replace("_", " ").title()})', fontsize=12
    )
    plt.xticks(rotation=45, ha="right", fontsize=10)
    ...
    file_path = os.path.join(
        out_dir,
        f"fees_analysis_by_{utils.sanitize_filename_component(x_label_text.lower())}_{utils.sanitize_filename_component(plot_col.lower())}{file_identifier}.png",
    )
    try:
        fig.savefig(file_path, dpi=300)
        logger.info(
            f"Gráfico de análisis de comisiones ({x_label_text}) guardado en: {file_path}"
        )
        return file_path
    except Exception as e_save_fees:
        logger.error(
            f"Error al guardar el gráfico de comisiones {file_path}: {e_save_fees}"
        )
        return None
    finally:
        plt.close(fig)
...
def plot_sankey_fiat_asset(
    ...
    if actual_value_col not in df_data.columns:
        logger.error(
            f"Columna de valor '{actual_value_col}' no encontrada en los datos para Sankey. Columnas disponibles: {df_data.columns}"
        )
        return None

    df_agg = df_data.group_by([fiat_col, asset_col]).agg(
        pl.sum(actual_value_col).alias(actual_value_col)
    )
    if df_agg.is_empty():
        logger.info(
            f"No hay datos agregados para generar el gráfico Sankey con la columna '{actual_value_col}'{title_suffix}."
        )
        return None
    ...
    combined_node_list = (
        sankey_agg_pd[fiat_col].tolist() + sankey_agg_pd[asset_col].tolist()
    )
    ...
    title_text = (
        f"Diagrama Sankey: Flujo Fiat -> Activo (Volumen en Fiat){title_suffix}"
    )
    fig.update_layout(title_text=title_text, font_size=10)

    file_path = os.path.join(
        out_dir, f"sankey_fiat_to_asset{file_identifier}.html"
    )  # Guardar como HTML para interactividad
    try:
...
def plot_heatmap_hour_day(
    df_pd: pd.DataFrame,
    out_dir: str,
    value_col_name: str = "TotalPrice_num",
    agg_func: str = "sum",
    title_suffix: str = "",
    file_identifier: str = "_general",
) -> str | None:
    if df_pd.empty:
        logger.info(
            f"No hay datos para el heatmap hora/día ({value_col_name} {agg_func}){title_suffix}."
        )
        return None

    logger.info(
        f"Generando heatmap hora/día para {value_col_name} (agg: {agg_func}){title_suffix}..."
    )

    if "Match_time_local" not in df_pd.columns:
        logger.warning(
            f"Columna 'Match_time_local' no encontrada en el DataFrame para heatmap. No se puede generar.{title_suffix}"
        )
        return None
    ...
    if not pd.api.types.is_datetime64_any_dtype(df_pd_copy["Match_time_local"]):
        try:
            df_pd_copy["Match_time_local"] = pd.to_datetime(
                df_pd_copy["Match_time_local"], errors="coerce"
            )
        except Exception as e:
            logger.error(
                f"Error al convertir 'Match_time_local' a datetime: {e}. No se puede generar heatmap.{title_suffix}"
            )
            return None

    if df_pd_copy["Match_time_local"].isna().all():
        logger.warning(
            f"'Match_time_local' contiene solo NaT después de la conversión. No se puede generar heatmap.{title_suffix}"
        )
        return None
    ...
    if (
        hasattr(df_pd_copy["Match_time_local"].dt, "tz")
        and df_pd_copy["Match_time_local"].dt.tz is not None
    ):
        try:
            logger.debug(
                f"Convirtiendo 'Match_time_local' a timezone-naive desde {df_pd_copy['Match_time_local'].dt.tz} para heatmap."
            )
            df_pd_copy["Match_time_local"] = df_pd_copy[
                "Match_time_local"
            ].dt.tz_convert(
                None
            )  # Convertir a UTC y hacerlo naive
            ...
            logger.error(
                f"Error al convertir 'Match_time_local' a timezone-naive: {e_tz_convert}. Usando datos como están si es posible."
            )
    ...
    try:
        ...
        logger.error(
            f"Error extrayendo componentes de hora/día de 'Match_time_local': {e_dt_extract}. Tipo de dato actual: {df_pd_copy['Match_time_local'].dtype}. No se puede generar heatmap."
        )
        return None

    # Extraer día de la semana (como número 0-6) y hora del día
    df_pd_copy["day_of_week_num"] = df_pd_copy[
        "Match_time_local"
    ].dt.dayofweek  # Lunes=0, Domingo=6
    df_pd_copy["day_of_week_label"] = df_pd_copy["day_of_week_num"].map(
        {
            0: "Monday",
            1: "Tuesday",
            2: "Wednesday",
    ...
    if value_col_name not in df_pd_copy.columns:
        logger.warning(
            f"Columna de valor '{value_col_name}' no encontrada para heatmap.{title_suffix}"
        )
        return None
    ...
    if agg_func == "count":
        ...
        if (
            value_col_name == "order_number"
            and value_col_name not in df_pd_copy.columns
        ):
            if df_pd_copy.columns.empty:
                return None  # No hay columnas
            value_col_name = df_pd_copy.columns[0]  # Usar cualquier columna existente
            logger.info(
                f"Columna '{value_col_name}' no encontrada para conteo, usando '{df_pd_copy.columns[0]}' como fallback."
            )
        ...
        if not pd.api.types.is_numeric_dtype(df_pd_copy[value_col_name]):
            logger.warning(
                f"Columna de valor '{value_col_name}' no es numérica para agg_func='sum' en heatmap. Intentando convertir..."
            )
            df_pd_copy[value_col_name] = pd.to_numeric(
                df_pd_copy[value_col_name], errors="coerce"
            )
            df_pd_copy = df_pd_copy.dropna(
                subset=[value_col_name]
            )  # Quitar filas donde la conversión falló
            if df_pd_copy.empty:
                logger.warning(
                    f"Columna de valor '{value_col_name}' quedó vacía después de intentar convertir a numérico.{title_suffix}"
                )
                return None
    else:
    ...
    try:
        ...
        logger.error(
            f"Error al crear pivot_table para heatmap ({value_col_name} {agg_func}): {e_pivot}"
        )
        return None
    ...
    pivot_table_ordered = pivot_table.reindex(
        columns=days_ordered_spanish, index=range(24)
    ).fillna(0)
    ...
    if final_agg_func == "size":
        ...
        try:
            ...
            logger.error(
                f"Error convirtiendo pivot_table_ordered a float para heatmap (sum): {e_astype_float}. Dtypes actuales:\n{pivot_table_ordered.dtypes}"
            )
    ...
    if pivot_table_ordered.empty:
        logger.info(
            f"Pivot table para heatmap resultó vacía ({value_col_name} {agg_func}).{title_suffix}"
        )
        return None
    ...
    sns.heatmap(
        ...
        annot=False,
        fmt=fmt_str,
        cbar_kws={"label": f"{agg_func.capitalize()} de {value_col_name}"},
    )
    plt.title(
        f"Heatmap de Actividad: Hora del Día vs. Día de la Semana ({agg_func.capitalize()} de {value_col_name}){title_suffix}"
    )
    ...
    file_path = os.path.join(
        out_dir,
        f"heatmap_hour_day_{clean_val_col_name}_{clean_agg_func}{file_identifier}.png",
    )
...
def plot_violin_price_vs_payment_method(
    df_data: pl.DataFrame,
    out_dir: str,
    title_suffix: str = "",
    file_identifier: str = "_general",
    ...
    saved_paths = []
    logger.info(
        f"Iniciando generación de gráficos de Violín: Precio vs Método de Pago{title_suffix}."
    )
    ...
    if not all(col in df_data.columns for col in required_cols):
        ...
        logger.warning(
            f"Faltan columnas ({', '.join(missing)}) para gráfico Violín Precio vs Método Pago{title_suffix}. No se generará."
        )
        return saved_paths
    ...
    df_violin_base = df_data.filter(
        ...
        & pl.col(payment_method_col).is_not_null()
        & pl.col(asset_col).is_not_null()
        & pl.col(fiat_col).is_not_null()
    )

    if df_violin_base.is_empty():
        logger.info(
            f"No hay datos válidos para gráfico Violín Precio vs Método de Pago{title_suffix}."
        )
        return saved_paths

    # Convertir a Pandas para Seaborn
    df_violin_pd = df_violin_base.select(required_cols).to_pandas(
        use_pyarrow_extension_array=True
    )

    # Agrupar por Asset y Fiat para generar gráficos separados
    for (current_asset, current_fiat), group_df in df_violin_pd.groupby(
        [asset_col, fiat_col]
    ):
        if group_df.empty or group_df[payment_method_col].nunique() < 1:
            logger.info(
                f"Omitiendo gráfico violín para {current_asset}/{current_fiat} (datos insuficientes o sin métodos de pago).{title_suffix}"
            )
            continue
        ...
        top_n_methods = 10
        common_methods = (
            group_df[payment_method_col].value_counts().nlargest(top_n_methods).index
        )
        df_plot = group_df[group_df[payment_method_col].isin(common_methods)]

        if df_plot.empty:
            logger.info(
                f"Omitiendo gráfico violín para {current_asset}/{current_fiat} (sin datos tras filtrar por Top N métodos).{title_suffix}"
            )
            continue

        plt.figure(figsize=(15, 8))
        order = sorted(
            df_plot[payment_method_col].unique()
        ...
        try:
            plt.savefig(file_path)
            logger.info(
                f"Gráfico Violín ({current_asset}/{current_fiat}) guardado en: {file_path}"
            )
            saved_paths.append(file_path)
            plt.close()
        except Exception as e:
            logger.error(
                f"Error al guardar Gráfico Violín ({current_asset}/{current_fiat}) {file_path}: {e}"
            )
            plt.close()
...
def plot_yoy_monthly_comparison(
    df_data: pl.DataFrame,
    out_dir: str,
    value_col: str,
    agg_func: str = "sum",
    ...
    saved_paths = []
    logger.info(
        f"Iniciando generación de gráfico YoY mensual ({value_col}, agg: {agg_func}){title_suffix}."
    )
    ...
    if not all(col in df_data.columns for col in required_cols_plot):
        ...
        logger.warning(
            f"Faltan columnas ({', '.join(missing)}) para gráfico YoY mensual{title_suffix}. No se generará."
        )
        return saved_paths

    df_plot_base = df_data.filter(
        pl.col(time_col).is_not_null() & pl.col(value_col).is_not_null()
    )
    if df_plot_base.is_empty():
        logger.info(
            f"No hay datos válidos para gráfico YoY mensual tras filtrar nulos{title_suffix}."
        )
        return saved_paths
    ...
    for (current_asset, current_fiat), group_df_asset_fiat in df_yoy_pd.groupby(
        [asset_col, fiat_col]
    ):
        ...
        try:
            ...
            logger.error(
                f"Error al pivotar datos para YoY ({current_asset}/{current_fiat}): {e_pivot_yoy}"
            )
            continue

        if pivot_yoy.empty:
            logger.info(
                f"Tabla pivote YoY vacía para {current_asset}/{current_fiat}{title_suffix}."
            )
            continue
        ...
        if pivot_yoy.columns.empty:  # No hay años para comparar
            logger.info(
                f"No hay suficientes años para comparación YoY para {current_asset}/{current_fiat}{title_suffix}."
            )
            continue
        ...
        plt.title(
            f"Comparación Mensual YoY de {agg_label} de {value_col} ({current_asset}/{current_fiat}){title_suffix}"
        )
        plt.xlabel("Mes")
        ...
        if len(month_labels) > 12:  # Aunque normalmente son 12 meses
            plt.gca().xaxis.set_major_locator(
                mticker.MaxNLocator(nbins=12, prune="both", integer=True)
            )
        ...
        try:
            plt.savefig(file_path)
            logger.info(
                f"Gráfico YoY ({current_asset}/{current_fiat}, {value_col}) guardado en: {file_path}"
            )
            saved_paths.append(file_path)
            plt.close()
        except Exception as e:
            logger.error(
                f"Error al guardar gráfico YoY ({current_asset}/{current_fiat}, {value_col}) {file_path}: {e}"
            )
            plt.close()
...
def plot_animated_scatter_price_volume(
    df_data: pl.DataFrame,
    out_dir: str,
    title_suffix: str = "",
    file_identifier: str = "_general",
    ...
    logger.info(
        f"Iniciando generación de Scatter animado Precio/Volumen{title_suffix}."
    )
    ...
    if not all(col in df_data.columns for col in required_cols_scatter):
        ...
        logger.warning(
            f"Faltan columnas ({', '.join(missing)}) para Scatter animado{title_suffix}. No se generará."
        )
        return None

    df_scatter = df_data.filter(
        pl.col(price_col).is_not_null()
        & pl.col(qty_col).is_not_null()
        & pl.col(time_col).is_not_null()
        & pl.col(asset_col).is_not_null()
        & pl.col(fiat_col).is_not_null()
        & pl.col(total_price_col).is_not_null()
    )

    if df_scatter.is_empty():
        logger.info(
            f"No hay datos válidos para Scatter animado tras filtrar nulos{title_suffix}."
        )
        return None
    ...
    if len(df_scatter_pd) > max_points_for_animation:
        logger.warning(
            f"Demasiados puntos ({len(df_scatter_pd)}) para scatter animado. Se tomará una muestra de {max_points_for_animation} puntos.{title_suffix}"
        )
        df_scatter_pd = df_scatter_pd.sample(
            n=max_points_for_animation, random_state=42
        )

    df_scatter_pd = df_scatter_pd.sort_values(
        by="animation_frame_date"
    ...
    try:
        fig = px.scatter(
            ...
            labels={
                ...
                qty_col: "Cantidad",
                asset_col: "Activo",
                total_price_col: "Valor Total Fiat",
            },
        )
        ...
        fig.layout.updatemenus[0].buttons[0].args[1]["frame"][
            "duration"
        ] = 500  # Duración de cada frame en ms
        ...
        logger.error(
            f"Error al generar Scatter animado con Plotly Express: {e_px_scatter}"
        )
        return None

    file_path = os.path.join(
        out_dir, f"scatter_animated_price_qty{file_identifier}.html"
    )
    try:
        fig.write_html(file_path)
...
def plot_boxplot_by_payment_method(
    ...
    saved_paths = []
    logger.info(
        f"Iniciando generación de Boxplots: '{value_col_label}' vs Método de Pago{title_suffix}."
    )
    ...
    if not all(col in df_data_pd.columns for col in required_cols):
        ...
        logger.warning(
            f"Faltan columnas ({', '.join(missing)}) para Boxplot '{value_col_label}' vs Método de Pago{title_suffix}. No se generarán gráficos."
        )
        return saved_paths
    ...
    if not pd.api.types.is_numeric_dtype(df_data_pd[value_col_name]):
        logger.warning(
            f"La columna de valor '{value_col_name}' no es numérica. No se pueden generar boxplots.{title_suffix}"
        )
        return saved_paths
    ...
    if df_plot_base.empty:
        logger.info(
            f"No hay datos válidos (después de eliminar NaNs en valor y método de pago) para Boxplot '{value_col_label}' vs Método de Pago{title_suffix}."
        )
        return saved_paths
    ...
    if asset_col and fiat_col:
        ...
        iterator = [
            (("Combined", "USD Equivalent"), df_plot_base)
        ]  # Crear un iterador de un solo grupo
        is_combined_plot = True

    for (current_asset, current_fiat), group_df in iterator:
        if group_df.empty or group_df[payment_method_col].nunique() < 1:
            logger.info(
                f"Omitiendo boxplot para {current_asset}/{current_fiat} (datos insuficientes o sin métodos de pago válidos).{title_suffix}"
            )
            continue
        ...
        if df_plot_final.empty:
            logger.info(
                f"Omitiendo boxplot para {current_asset}/{current_fiat} (sin datos tras filtrar por Top {top_n_methods} métodos).{title_suffix}"
            )
            continue
        ...
        fig_width = max(
            12, num_methods * 0.8
        )  # Ajustar ancho de la figura según cantidad de métodos
        ...
        try:
            plt.savefig(file_path)
            logger.info(
                f"Boxplot ({current_asset}/{current_fiat} - {value_col_label}) guardado en: {file_path}"
            )
            saved_paths.append(file_path)
            plt.close()
        except Exception as e:
            logger.error(
                f"Error al guardar Boxplot ({current_asset}/{current_fiat} - {value_col_label}) {file_path}: {e}"
            )
            plt.close()
...
def plot_order_status_over_time(
    ...
    logger.info(
        f"Iniciando generación de gráfico de Completitud de Órdenes a lo largo del Tiempo{title_suffix}."
    )

    required_cols = [time_col, status_col]
    if not all(col in df_all_statuses_pd.columns for col in required_cols):
        missing = [
            col for col in required_cols if col not in df_all_statuses_pd.columns
        ]
        logger.warning(
            f"Faltan columnas ({', '.join(missing)}) para gráfico de Completitud de Órdenes{title_suffix}. No se generará."
        )
        return None

    if df_all_statuses_pd[required_cols].isna().any().any():
        logger.warning(
            f"Hay valores nulos en columnas '{time_col}' o '{status_col}'. Se intentará continuar filtrando esos nulos."
        )
    ...
    if not pd.api.types.is_datetime64_any_dtype(df_plot_data[time_col]):
        try:
            ...
            logger.error(
                f"No se pudo convertir la columna de tiempo '{time_col}' a datetime: {e}. No se generará gráfico."
            )
            return None

    if df_plot_data.empty:
        logger.info(
            f"No hay datos válidos para gráfico de Completitud de Órdenes{title_suffix}."
        )
        return None
    ...
    status_counts_monthly = (
        df_plot_data.groupby(["YearMonth", status_col]).size().unstack(fill_value=0)
    )

    if status_counts_monthly.empty:
        logger.info(
            f"No hay datos después de agrupar por mes y estado para Completitud de Órdenes{title_suffix}."
        )
        return None

    # Calcular porcentajes
    status_percentages_monthly = status_counts_monthly.apply(
    ...
    status_order = [
        ...
        "Cancelled",
        "Appealing",
    ]  # Definir un orden deseado para el stack
    ...
    other_cols = [
        s for s in status_percentages_monthly.columns if s not in cols_to_plot
    ]
    final_plot_order = cols_to_plot + other_cols

    # Crear el gráfico de áreas apiladas
    status_percentages_monthly[final_plot_order].plot(
        kind="area", stacked=True, colormap="viridis", alpha=0.8
    )

    plt.title(
        f"Distribución Porcentual de Estados de Órdenes por Mes{title_suffix}",
        fontsize=16,
    )
    ...
    try:
        plt.savefig(file_path)
        logger.info(
            f"Gráfico de Completitud de Órdenes por Mes guardado en: {file_path}"
        )
        plt.close()
        return file_path
    except Exception as e:
        logger.error(
            f"Error al guardar gráfico de Completitud de Órdenes por Mes {file_path}: {e}"
        )
        plt.close()
        return None
...
def plot_volume_by_day_of_week(
    ...
    specific_plot_title: (
        str | None
    ) = None,  # Título específico si se quiere sobreescribir el default
) -> str | None:
    logger.info(
        f"Iniciando generación de gráfico de Volumen por Día de la Semana ({volume_col_label}){title_suffix}."
    )
    ...
    if not all(col in df_data_pd.columns for col in required_cols):
        missing = [col for col in required_cols if col not in df_data_pd.columns]
        logger.warning(
            f"Faltan columnas ({', '.join(missing)}) para gráfico de Volumen por Día de la Semana{title_suffix}. No se generará."
        )
        return None
    ...
    if not pd.api.types.is_datetime64_any_dtype(df_plot_data[time_col]):
        try:
            ...
            logger.error(
                f"No se pudo convertir la columna de tiempo '{time_col}' a datetime: {e}. No se generará gráfico de volumen por día."
            )
            return None

    if not pd.api.types.is_numeric_dtype(df_plot_data[volume_col]):
        logger.error(
            f"La columna de volumen '{volume_col}' no es numérica. No se generará gráfico de volumen por día."
        )
        return None

    if df_plot_data.empty:
        logger.info(
            f"No hay datos válidos para gráfico de Volumen por Día de la Semana ({volume_col_label}){title_suffix}."
        )
        return None
    ...
    df_plot_data["day_of_week"] = pd.Categorical(
        df_plot_data["day_of_week"], categories=days_ordered, ordered=True
    )

    volume_by_day = df_plot_data.groupby("day_of_week", observed=False)[
        volume_col
    ].sum()

    if volume_by_day.empty or volume_by_day.sum() == 0:
        logger.info(
            f"No hay volumen agregado para graficar por día de la semana ({volume_col_label}){title_suffix}."
        )
        return None
    ...
    title = (
        ...
        else f"Volumen Total de {volume_col_label} por Día de la Semana{title_suffix}"
    )
    ...
    try:
        plt.savefig(file_path)
        logger.info(
            f"Gráfico de Volumen por Día de la Semana ({volume_col_label}) guardado en: {file_path}"
        )
        plt.close()
        return file_path
    except Exception as e:
        logger.error(
            f"Error al guardar gráfico de Volumen por Día de la Semana ({volume_col_label}) {file_path}: {e}"
        )
        plt.close()
        return None
...
def plot_buy_sell_volume_over_time(
    ...
    logger.info(
        f"Iniciando generación de gráfico de Volumen Compra vs. Venta ({volume_col_label}){title_suffix}."
    )
    ...
    if not all(col in df_data_pd.columns for col in required_cols):
        missing = [col for col in required_cols if col not in df_data_pd.columns]
        logger.warning(
            f"Faltan columnas ({', '.join(missing)}) para gráfico Volumen Compra vs. Venta{title_suffix}. No se generará."
        )
        return None
    ...
    if not pd.api.types.is_numeric_dtype(df_plot[volume_col]):
        try:
            ...
            logger.error(
                f"No se pudo convertir la columna de volumen '{volume_col}' a numérica: {e_conv_num_vol}. No se generará gráfico."
            )
            return None

    if df_plot.empty:
        logger.info(
            f"No hay datos válidos para gráfico Volumen Compra vs. Venta ({volume_col_label}){title_suffix}."
        )
        return None
    ...
    if df_plot.empty:
        logger.info(
            f"No hay datos con tipos de orden 'BUY' o 'SELL' para gráfico Volumen Compra vs. Venta ({volume_col_label}){title_suffix}."
        )
        return None

    volume_by_month_type = (
        df_plot.groupby(["YearMonth", order_type_col], observed=False)[volume_col]
        .sum()
    ...
    if volume_by_month_type.empty:
        logger.info(
            f"No hay volumen agregado para graficar Compra vs. Venta ({volume_col_label}){title_suffix}."
        )
        return None

    # Asegurar que las columnas BUY y SELL existan, incluso si no hubo operaciones de un tipo
    if "BUY" not in volume_by_month_type.columns:
        volume_by_month_type["BUY"] = 0
    if "SELL" not in volume_by_month_type.columns:
        volume_by_month_type["SELL"] = 0

    # Convertir PeriodIndex a string para graficar
    volume_by_month_type.index = volume_by_month_type.index.astype(str)

    plt.figure(figsize=(15, 8))
    volume_by_month_type[["BUY", "SELL"]].plot(
        kind="bar", stacked=False, colormap="coolwarm"
    )  # side-by-side
    ...
    title = (
        specific_plot_title
        if specific_plot_title
        else f"Volumen Mensual de Compra vs. Venta ({volume_col_label}){title_suffix}"
    )
    ...
    try:
        plt.savefig(file_path)
        logger.info(
            f"Gráfico de Volumen Compra vs. Venta ({volume_col_label}) guardado en: {file_path}"
        )
        plt.close()
        return file_path
    except Exception as e:
        logger.error(
            f"Error al guardar gráfico de Volumen Compra vs. Venta ({volume_col_label}) {file_path}: {e}"
        )
        plt.close()
        return None
...
def plot_simplified_market_depth(
    ...
    saved_paths = []
    logger.info(
        f"Iniciando generación de gráfico de Profundidad de Mercado Simplificado{title_suffix}."
    )
    ...
    if not all(col in df_data_pd.columns for col in required_cols):
        missing = [col for col in required_cols if col not in df_data_pd.columns]
        logger.warning(
            f"Faltan columnas ({', '.join(missing)}) para Profundidad de Mercado{title_suffix}. No se generará."
        )
        return saved_paths
    ...
    for col_num in [price_col, quantity_col]:
        if not pd.api.types.is_numeric_dtype(df_plot_data[col_num]):
            try:
                df_plot_data[col_num] = pd.to_numeric(
                    df_plot_data[col_num], errors="coerce"
                )
                df_plot_data.dropna(subset=[col_num], inplace=True)
            except Exception as e_conv_md_num:
                logger.error(
                    f"No se pudo convertir la columna '{col_num}' a numérica para Market Depth: {e_conv_md_num}."
                )
                return saved_paths
    ...
    for (current_asset, current_fiat), group_df_pair in df_plot_data.groupby(
        [asset_col, fiat_col]
    ):
        if group_df_pair.empty:
            logger.info(
                f"No hay datos para el par {current_asset}/{current_fiat} para Profundidad de Mercado."
            )
            continue
        ...
        if bids.empty and asks.empty:
            logger.info(
                f"No hay órdenes BUY o SELL para {current_asset}/{current_fiat} para Profundidad de Mercado."
            )
            continue
        ...
        if not bids.empty:
            bid_depth = (
                bids.groupby(price_col)[quantity_col]
                .sum()
            ...
            bid_depth.rename(
                columns={quantity_col: "cumulative_bid_volume"}, inplace=True
            )
        ...
        if not asks.empty:
            ask_depth = (
                asks.groupby(price_col)[quantity_col]
                .sum()
            ...
            ask_depth.rename(
                columns={quantity_col: "cumulative_ask_volume"}, inplace=True
            )

        if bid_depth.empty and ask_depth.empty:
            logger.info(
                f"No se pudo calcular profundidad para Bids ni Asks para {current_asset}/{current_fiat}."
            )
            continue
        ...
        if not bid_depth.empty:
            plt.plot(
                bid_depth["cumulative_bid_volume"],
                bid_depth[price_col],
                label="Demanda (Vol. Compras Acum.)",
                color="green",
                drawstyle="steps-pre",
            )
        ...
        if not ask_depth.empty:
            plt.plot(
                ask_depth["cumulative_ask_volume"],
                ask_depth[price_col],
                label="Oferta (Vol. Ventas Acum.)",
                color="red",
                drawstyle="steps-pre",
            )

        plt.title(
            f"Profundidad de Mercado Simplificada para {current_asset}/{current_fiat}{title_suffix}",
            fontsize=15,
        )
        ...
        ax.yaxis.set_major_formatter(
            mticker.FuncFormatter(
                lambda x, p: utils.format_large_number(x) if x > 1000 else f"{x:.3f}"
            )
        )
        ...
        try:
            plt.savefig(file_path)
            logger.info(
                f"Gráfico de Profundidad de Mercado ({current_asset}/{current_fiat}) guardado en: {file_path}"
            )
            saved_paths.append(file_path)
            plt.close()
        except Exception as e:
            logger.error(
                f"Error al guardar Profundidad de Mercado ({current_asset}/{current_fiat}) {file_path}: {e}"
            )
            plt.close()
...
def plot_daily_average(
    ...
    if fiat_type_col_internal in daily_avg_fiat.columns:
        ...
        isinstance(daily_avg_fiat.index, pd.MultiIndex)
        and fiat_type_col_internal in daily_avg_fiat.index.names
    ):
        unique_fiats = daily_avg_fiat.index.get_level_values(
            fiat_type_col_internal
        ).unique()
    ...
    for i, current_fiat in enumerate(unique_fiats):
        ...
        if current_fiat is not None:
            ...
            if fiat_column_for_grouping:
                df_plot_full_orig = daily_avg_fiat[
                    daily_avg_fiat[fiat_column_for_grouping] == current_fiat
                ]
            elif (
                isinstance(daily_avg_fiat.index, pd.MultiIndex)
                and fiat_type_col_internal in daily_avg_fiat.index.names
                ...
                df_plot_full_orig = daily_avg_fiat[
                    daily_avg_fiat.index.get_level_values(fiat_type_col_internal)
                    == current_fiat
                ]
            elif daily_avg_fiat.index.name == fiat_type_col_internal:
        ...
        if df_plot_full_orig.empty:
            logger.info(
                f"Datos vacíos para Fiat: {current_fiat_label_for_title} en plot_daily_average. Omitiendo."
            )
            continue

        # Asegurar que DayOfWeek sea una columna para facilitar el ploteo
        if (
            isinstance(df_plot_full_orig.index, pd.MultiIndex)
            and day_of_week_col_internal in df_plot_full_orig.index.names
        ):
        ...
        if day_of_week_col_internal not in df_plot_full.columns:
            logger.warning(
                f"La columna '{day_of_week_col_internal}' no fue encontrada en los datos para plot_daily_average para {current_fiat_label_for_title}{title_suffix}."
            )
            continue
        ...
        if pd.api.types.is_numeric_dtype(df_plot_full[day_of_week_col_internal]):
            ...
            df_plot_full["DayName"] = df_plot_full[day_of_week_col_internal].map(
                day_map_num_to_name
            )
            plot_x_col = "DayName"
            ...
            category_order = pd.Categorical(
                df_plot_full[day_of_week_col_internal],
        ...
        df_plot_full = df_plot_full.sort_values(
            by=plot_x_col,
            key=lambda x: (
                x.map({day: i for i, day in enumerate(days_order_es)})
                if pd.api.types.is_string_dtype(x)
                else x
            ),
        )

        exclude_cols_from_values = {
            day_of_week_col_internal,
            fiat_type_col_internal,
            "DayName",
        }
        potential_value_cols = [
            col
            for col in df_plot_full.columns
            if col not in exclude_cols_from_values
            and pd.api.types.is_numeric_dtype(df_plot_full[col])
        ]

        if not potential_value_cols:
            logger.warning(
                f"No hay columnas de valor numéricas para graficar en plot_daily_average para {current_fiat_label_for_title}{title_suffix}. Columnas: {df_plot_full.columns}"
            )
            continue
        ...
        x_indices = np.arange(
            len(df_plot_full[plot_x_col].unique())
        )  # Un índice numérico para las posiciones de las barras

        for idx, value_col in enumerate(potential_value_cols):
            series_to_plot = (
                df_plot_full.groupby(plot_x_col, observed=False)[value_col]
                .mean()
            ...
            plt.axhline(
                ...
                linestyle="--",
                alpha=0.7,
                label=f"{value_col} (Media Gral: {utils.format_large_number(overall_mean)})",
            )
        ...
        plt.xlabel("Día de la Semana", fontsize=12)
        plt.ylabel(
            f"Volumen Promedio Transaccionado ({current_fiat_label_for_title})",
            fontsize=12,
        )
        ...
        plt.grid(
            True, linestyle="--", alpha=0.7, axis="y"
        )  # Grid solo en el eje Y para barplot
        plt.tight_layout(rect=[0, 0, 1, 0.95])

        fiat_file_part = f"_{current_fiat_label_for_file.lower()}_"
        file_path = os.path.join(
            out_dir, f"daily_average_volume{fiat_file_part}{file_identifier}.png"
        )
        try:
            plt.savefig(file_path, dpi=300)
            logger.info(
                f"Gráfico de volumen promedio diario ({current_fiat_label_for_title}) guardado en: {file_path}"
            )
            saved_paths.append(file_path)
        except Exception as e:
