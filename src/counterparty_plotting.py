...
def generate_all_counterparty_plots(
    ...
    try:
        ...
        if 'general_stats' in counterparty_data and not counterparty_data['general_stats'].empty:
        ...
        if 'vip_counterparties' in counterparty_data and not counterparty_data['vip_counterparties'].empty:
        ...
        if 'payment_preferences' in counterparty_data and not counterparty_data['payment_preferences'].empty:
        ...
        if 'temporal_evolution' in counterparty_data and not counterparty_data['temporal_evolution'].empty:
        ...
        if 'efficiency_stats' in counterparty_data and 'general_stats' in counterparty_data:
            if not counterparty_data['efficiency_stats'].empty and not counterparty_data['general_stats'].empty:
        ...
        if 'trading_patterns' in counterparty_data and not counterparty_data['trading_patterns'].empty:
...
def plot_counterparty_volume_ranking(
    ...
    try:
        ...
        ax.set_title(f'Top {top_n} Contrapartes por Volumen{title_suffix}', fontsize=14, fontweight='bold')
        
        # Añadir valores en las barras
        for i, (bar, value) in enumerate(zip(bars, top_counterparties['total_volume'])):
            ax.text(value + max(top_counterparties['total_volume']) * 0.01, 
...
def plot_volume_vs_frequency_scatter(
    ...
    try:
        ...
        ax.set_title(f'Volumen vs Frecuencia de Trading{title_suffix}', fontsize=14, fontweight='bold')
        
        # Escala logarítmica si es necesario
        if general_stats_df['total_volume'].max() / general_stats_df['total_volume'].min() > 100:
            ax.set_yscale('log')
        if general_stats_df['total_operations'].max() / general_stats_df['total_operations'].min() > 50:
            ax.set_xscale('log')
...
def plot_vip_tier_distribution(
    ...
    try:
        ...
        ax1.set_title(f'Distribución de Tiers VIP{title_suffix}', fontsize=14, fontweight='bold')
...
def plot_payment_preferences_heatmap(
    ...
    try:
        ...
        top_counterparties = payment_df.groupby('Counterparty')['volume_with_method'].sum().nlargest(15).index
        pivot_filtered = pivot_data.loc[pivot_data.index.intersection(top_counterparties)]
        ...
        ax.set_title(f'Preferencias de Métodos de Pago por Contraparte{title_suffix}', 
...
def plot_temporal_timeline(
    ...
    try:
        ...
        total_volume_by_cp = temporal_df.groupby('Counterparty')['monthly_volume'].sum().nlargest(8)
        ...
        for i, cp in enumerate(top_counterparties):
            cp_data = filtered_df[filtered_df['Counterparty'] == cp].sort_values('year_month')
        ...
        ax.set_title(f'Evolución Temporal de Contrapartes Top{title_suffix}', fontsize=14, fontweight='bold')
...
def plot_efficiency_vs_volume(
    ...
    try:
        ...
        ax.set_title(f'Eficiencia vs Volumen por Contraparte{title_suffix}', fontsize=14, fontweight='bold')
        ...
        ax.axhline(y=combined_df['efficiency_score'].median(), color='red', linestyle='--', alpha=0.5, label='Mediana Eficiencia')
        ax.axvline(x=combined_df['total_volume'].median(), color='blue', linestyle='--', alpha=0.5, label='Mediana Volumen')
        ax.legend()
...
def plot_trading_patterns_radar(
    ...
    try:
        ...
        numeric_cols = ['most_active_hour', 'hour_spread', 'unique_hour_day_combinations']
        for col in numeric_cols:
            if col in df_processed.columns:
                df_processed[col] = pd.to_numeric(df_processed[col], errors='coerce')
            else:
                logger.warning(f"Columna '{col}' no encontrada en patterns_df para gráfico radar. Se omitirá.")
        ...
        if 'most_active_weekday' in df_processed.columns:
            df_processed['most_active_weekday_numeric'] = pd.to_numeric(df_processed['most_active_weekday'], errors='coerce').fillna(0).astype(int)
        else:
            logger.warning("Columna 'most_active_weekday' no encontrada en patterns_df para gráfico radar. Se usará 0.")
        ...
        if sort_col not in df_processed.columns or not pd.api.types.is_numeric_dtype(df_processed[sort_col]):
            logger.warning(f"Columna de ordenamiento '{sort_col}' no es numérica o no existe para radar. Se tomarán las primeras {top_n} filas.")
            top_n_df = df_processed.head(top_n)
        else:
            top_n_df = df_processed.sort_values(by=sort_col, ascending=False).head(top_n)

        if top_n_df.empty:
            logger.warning("No hay datos suficientes en patterns_df después del preprocesamiento para generar radar.")
            return saved_paths
        ...
        for index, row in top_n_df.iterrows():
            ...
            values.append(row['most_active_hour'] / 24.0 if pd.notnull(row['most_active_hour']) else 0)
            # 2. Día Más Activo (normalizado, sobre 7 días)
            values.append(row['most_active_weekday_numeric'] / 7.0 if pd.notnull(row['most_active_weekday_numeric']) else 0)
            ...
            values.append(row['hour_spread'] / max_hour_spread_observed if pd.notnull(row['hour_spread']) and max_hour_spread_observed > 0 else 0)
            # 4. Combinaciones Hora/Día Únicas (normalizada, ej. sobre max combinaciones observadas)
            max_combinations_observed = df_processed['unique_hour_day_combinations'].max()
            values.append(row['unique_hour_day_combinations'] / max_combinations_observed if pd.notnull(row['unique_hour_day_combinations']) and max_combinations_observed > 0 else 0)

            fig.add_trace(go.Scatterpolar(
                r=values,
                theta=categories,
                fill='toself',
                name=str(row.get('Counterparty', f"CP_{index}")) # Usar 'Counterparty' si existe
            ))
