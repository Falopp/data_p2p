...
def analyze_trading_sessions(
    df: pl.DataFrame, session_gap_minutes: int = 30
    ...
    """
    logger.info(
        f"Iniciando análisis de sesiones de trading (gap: {session_gap_minutes} min)..."
    )
    ...
    logger.info(
        f"Analizando {df_sessions.height} operaciones para identificar sesiones..."
    )
...
def _identify_sessions(df: pl.DataFrame, gap_minutes: int) -> pl.DataFrame:
    ...
    df_pandas["new_session"] = (df_pandas["time_diff_minutes"] > gap_minutes) | (
        df_pandas["time_diff_minutes"].isna()
    )
    ...
    session_stats = (
        ...
        .with_columns(
            [
                ...
                (
                    pl.col("num_operations")
                    / (pl.col("duration_minutes") / 60).clip(lower_bound=0.01)
                ).alias("operations_per_hour"),
                # Volumen por minuto
                (
                    pl.col("total_volume")
                    / pl.col("duration_minutes").clip(lower_bound=0.01)
                ).alias("volume_per_minute"),
            ]
        )
        .sort("session_start")
    )
...
def _analyze_session_patterns(df: pl.DataFrame) -> pl.DataFrame:
    ...
    hourly_patterns = (
        df_with_time.group_by(["session_id", "hour"])
        .agg(
            [
    ...
    efficiency_stats = (
        ...
        .with_columns(
            [
                ...
                (
                    1
                    / (pl.col("volume_volatility") / pl.col("total_volume")).clip(
                        lower_bound=0.01
                    )
...
def _analyze_counterparty_sessions(df: pl.DataFrame) -> pl.DataFrame:
    ...
    counterparty_session_stats = (
        df.group_by(["Counterparty", "session_id"])
        .agg(
            [
        ...
        .agg(
            [
                ...
                (
                    (
                        pl.col("last_op_in_session") - pl.col("first_op_in_session")
                    ).dt.total_seconds()
                    / 60
                )
                .mean()
                .alias("avg_session_duration_minutes"),
            ]
        )
...
def _analyze_temporal_distribution(df: pl.DataFrame) -> pl.DataFrame:
    ...
    hourly_distribution = (
        temporal_stats.group_by("start_hour")
        .agg(
            [
...
def get_session_insights(session_data: Dict[str, pl.DataFrame]) -> Dict[str, Any]:
    ...
    if "counterparty_sessions" in session_data:
        ...
        if not cp_stats.is_empty():
            insights.update(
                {
                    "most_active_counterparty_sessions": cp_stats[
                        "total_sessions"
                    ].max(),
