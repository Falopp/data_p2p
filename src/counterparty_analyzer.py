...
def analyze_counterparties(df: pl.DataFrame) -> Dict[str, Union[pl.DataFrame, Any]]:
    ...
    required_cols = ['Counterparty', 'TotalPrice_num', 'Price_num', 'Quantity_num', 
                    'Match_time_local', 'payment_method', 'status', 'order_type']
    ...
    counterparty_metrics['general_stats'] = _calculate_general_counterparty_stats(df_filtered)
    
    # 2. Evolución temporal de contrapartes
    logger.info("Analizando evolución temporal de contrapartes...")
    counterparty_metrics['temporal_evolution'] = _analyze_temporal_evolution(df_filtered)
    
    # 3. Métodos de pago preferidos por contraparte
    logger.info("Analizando métodos de pago por contraparte...")
    counterparty_metrics['payment_preferences'] = _analyze_payment_preferences(df_filtered)
    ...
    counterparty_metrics['vip_counterparties'] = _identify_vip_counterparties(df_filtered)
...
def _calculate_general_counterparty_stats(df: pl.DataFrame) -> pl.DataFrame:
    ...
    return (
        ...
        .agg([
            ...
            (pl.max('Match_time_local') - pl.min('Match_time_local')).dt.total_days().alias('days_active'),
        ])
        .with_columns([
            # Calcular frecuencia de trading (operaciones por día)
            (pl.col('total_operations') / (pl.col('days_active') + 1)).alias('operations_per_day'),
            # Calcular volumen por día
            (pl.col('total_volume') / (pl.col('days_active') + 1)).alias('volume_per_day'),
...
def _analyze_payment_preferences(df: pl.DataFrame) -> pl.DataFrame:
    ...
    payment_stats = (
        df.group_by(['Counterparty', 'payment_method'])
        .agg([
    ...
    return (
        payment_stats
        .join(counterparty_totals, on='Counterparty')
        .with_columns([
            (pl.col('operations_with_method') / pl.col('total_operations_cp') * 100).alias('pct_operations'),
            (pl.col('volume_with_method') / pl.col('total_volume_cp') * 100).alias('pct_volume'),
        ])
...
def _analyze_trading_patterns(df: pl.DataFrame) -> pl.DataFrame:
    ...
    df_with_time_parts = df.with_columns([
        ...
        pl.col('Match_time_local').dt.weekday().alias('weekday'), # Lunes=1, Domingo=7
    ])
    ...
    try:
        weekday_stats_raw = df_with_time_parts.group_by('Counterparty').agg([
            ...
            pl.col('weekday').value_counts(sort=True).alias('weekday_value_counts_struct') # Mantener como struct para inspección
        ])
        ...
        logger.info(f"Estadísticas de 'weekday' cruda por Counterparty (value_counts es un struct):\n{weekday_stats_raw}")

        problematic_weekdays = weekday_stats_raw.filter(
            (pl.col('min_weekday') < 1) | (pl.col('max_weekday') > 7) | 
            ((pl.col('null_weekday_count') > 0) & (pl.col('valid_weekday_count') == 0))
        )
        if not problematic_weekdays.is_empty():
            logger.warning(f"ALERTA: Se encontraron valores de 'weekday' problemáticos (fuera de 1-7 o todos nulos) para algunas contrapartes:\n{problematic_weekdays}")
    except Exception as e_log_wk:
        logger.error(f"Error al loguear estadísticas de 'weekday': {e_log_wk}")

    aggregated_patterns = (
        df_with_time_parts
        .group_by('Counterparty') # Agrupación directa por Counterparty
        .agg([
            pl.col('hour').filter(pl.col('hour').is_not_null()).mode().first().alias('most_active_hour'),
            # Dejar most_active_weekday como Int8 (0-7)
            pl.col('weekday').filter(pl.col('weekday').is_not_null()).mode().first().fill_null(0).cast(pl.Int8).alias('most_active_weekday_int'),
            (pl.col('hour').filter(pl.col('hour').is_not_null()).max() - pl.col('hour').filter(pl.col('hour').is_not_null()).min()).alias('hour_spread'),
            pl.struct([
                pl.col('hour').filter(pl.col('hour').is_not_null()), 
                pl.col('weekday').filter(pl.col('weekday').is_not_null())
            ]).alias('hour_weekday_struct').drop_nulls().n_unique().alias('unique_hour_day_combinations'),
        ])
    )

    # Log de diagnóstico para 'most_active_weekday_int' antes del mapeo
    try:
        most_active_weekday_int_values = aggregated_patterns.select(['Counterparty', 'most_active_weekday_int']).drop_nulls('most_active_weekday_int')
        logger.info(f"Valores de 'most_active_weekday_int' (resultado del mode(), tipo Int8) antes del mapeo a nombres:\\n{most_active_weekday_int_values}")
        
        problematic_int_modes = most_active_weekday_int_values.filter(
            (pl.col('most_active_weekday_int') < 0) | (pl.col('most_active_weekday_int') > 7)
        )
        if not problematic_int_modes.is_empty():
            logger.warning(f"ALERTA: Se encontraron valores de 'most_active_weekday_int' problemáticos (fuera de 0-7) DESPUÉS del mode():\\n{problematic_int_modes}")

    except Exception as e_log_maw:
        logger.error(f"Error al loguear valores de 'most_active_weekday_int': {e_log_maw}")

    # Asegurar que las columnas en aggregated_patterns tengan tipos simples antes del 'replace'
    try:
        logger.info(f"Schema de aggregated_patterns ANTES de casteo explícito: {aggregated_patterns.schema}")
        aggregated_patterns = aggregated_patterns.select([
            pl.col("Counterparty"), # Debe ser String
            pl.col("most_active_hour").cast(pl.Int32, strict=False).alias("most_active_hour"),
            pl.col("most_active_weekday_int"), # Ya es Int8
            pl.col("hour_spread").cast(pl.Int32, strict=False).alias("hour_spread"),
            pl.col("unique_hour_day_combinations").cast(pl.UInt32, strict=False).alias("unique_hour_day_combinations")
        ])
        logger.info(f"Schema de aggregated_patterns DESPUÉS de casteo explícito y re-select: {aggregated_patterns.schema}")
    except Exception as e_cast:
        logger.error(f"Error durante el casteo explícito de aggregated_patterns: {e_cast}")
    ...
    try:
        unique_values_in_problem_col = aggregated_patterns.select(pl.col('most_active_weekday_int').unique().sort()).to_series().to_list()
        logger.info(f"Valores únicos en 'most_active_weekday_int' justo antes de mapeo when/then: {unique_values_in_problem_col}")
        logger.info(f"Tipo de la columna 'most_active_weekday_int': {aggregated_patterns.get_column('most_active_weekday_int').dtype}")

        # Mapeo usando when/then/otherwise
        weekday_name_expr = (
            pl.when(pl.col('most_active_weekday_int') == 1).then(pl.lit('Lunes'))
            .when(pl.col('most_active_weekday_int') == 2).then(pl.lit('Martes'))
            .when(pl.col('most_active_weekday_int') == 3).then(pl.lit('Miércoles'))
            .when(pl.col('most_active_weekday_int') == 4).then(pl.lit('Jueves'))
            .when(pl.col('most_active_weekday_int') == 5).then(pl.lit('Viernes'))
            .when(pl.col('most_active_weekday_int') == 6).then(pl.lit('Sábado'))
            .when(pl.col('most_active_weekday_int') == 7).then(pl.lit('Domingo'))
            .when(pl.col('most_active_weekday_int') == 0).then(pl.lit('Desconocido (sin moda)'))
            .otherwise(pl.lit('FALLO_MAPEO_OTHERWISE')) # Default para valores inesperados
            .cast(pl.String) # Asegurar que la columna final sea String
            .alias('most_active_weekday_name')
        )
        
        logger.info("Expresión para 'most_active_weekday_name' (con when/then) creada. Intentando aplicarla con with_columns...")
        
        result_df = aggregated_patterns.with_columns(weekday_name_expr)
        logger.info(f"Columna 'most_active_weekday_name' (con when/then) añadida exitosamente. Schema resultante: {result_df.schema}")
        return result_df

    except Exception as e_whenthen_detailed:
        logger.error(f"ERROR DETALLADO durante when/then o al añadir columna 'most_active_weekday_name': {e_whenthen_detailed}")
        logger.error(f"Tipo de error detallado (when/then): {type(e_whenthen_detailed)}")
        logger.warning("Devolviendo aggregated_patterns SIN 'most_active_weekday_name' debido a error en su creación (when/then).")
...
def _identify_vip_counterparties(df: pl.DataFrame) -> pl.DataFrame:
    ...
    return (
        ...
        .with_columns([
            ...
            (pl.col('total_operations') >= operations_p90).alias('high_frequency_vip'),
        ...
        .with_columns([
            # Clasificación VIP
            pl.when(pl.col('vip_score') >= 80).then(pl.lit('Diamond'))
            .when(pl.col('vip_score') >= 60).then(pl.lit('Gold'))
            .when(pl.col('vip_score') >= 40).then(pl.lit('Silver'))
            .when(pl.col('vip_score') >= 20).then(pl.lit('Bronze'))
...
def _calculate_efficiency_stats(df: pl.DataFrame) -> pl.DataFrame:
    ...
    return (
        ...
        .agg([
            ...
            # Tasa de completitud
            (pl.col('status').filter(pl.col('status') == 'Completed').count() / pl.count('Counterparty') * 100).alias('completion_rate'),
            # Tasa de cancelación
            (pl.col('status').filter(pl.col('status').str.contains('Cancel')).count() / pl.count('Counterparty') * 100).alias('cancellation_rate'),
            # Variabilidad en tiempos (si tienes timestamps)
            pl.col('Match_time_local').diff().dt.total_hours().std().alias('timing_variability_hours'),
            # Consistencia en volúmenes
            (pl.std('TotalPrice_num') / pl.mean('TotalPrice_num')).alias('volume_consistency'),
        ])
        .with_columns([
            # Score de eficiencia (0-100)
            (
                pl.col('completion_rate') * 0.5 +  # 50% peso en completitud
                (100 - pl.col('cancellation_rate')) * 0.3 +  # 30% peso en no cancelación
                (100 - pl.col('volume_consistency').clip(0, 100)) * 0.2  # 20% peso en consistencia
...
def get_counterparty_insights(counterparty_df: pl.DataFrame) -> Dict[str, Any]:
    ...
    if counterparty_df is None or counterparty_df.is_empty():
        return {"error": "No hay datos consolidados de contrapartes para generar insights"}
    ...
    if 'total_volume' in counterparty_df.columns:
        insights['top_counterparty_by_volume'] = counterparty_df.sort('total_volume', descending=True).row(0, named=True)
        ...
        if total_volume_sum is not None and total_volume_sum > 0 :
            ...
            insights['top_10_volume_concentration_pct'] = (top_10_volume_sum / total_volume_sum * 100) if total_volume_sum > 0 else 0
        else:
    ...
    if 'vip_tier' in counterparty_df.columns:
        insights['vip_breakdown'] = counterparty_df.group_by('vip_tier').agg(pl.count().alias('count')).to_dict(as_series=False)
    
    if 'total_operations' in counterparty_df.columns:
        insights['avg_operations_per_counterparty'] = counterparty_df['total_operations'].mean()
