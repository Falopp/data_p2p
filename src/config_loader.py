    ...
    if config_path and os.path.exists(config_path):
        try:
            ...
            if loaded_config and isinstance(loaded_config, dict):
                ...
                logger.warning(f"El archivo de configuración '{config_path}' está vacío o no es un diccionario válido. Usando configuración por defecto.")
        except yaml.YAMLError as e:
            logger.error(f"Error parseando el archivo de configuración YAML '{config_path}': {e}. Usando configuración por defecto.")
        except Exception as e:
            logger.error(f"Error inesperado cargando el archivo de configuración '{config_path}': {e}. Usando configuración por defecto.")
    elif config_path:
        logger.warning(f"Archivo de configuración no encontrado en '{config_path}'. Usando configuración por defecto.")
    
    logger.info("Usando configuración por defecto del script (ya sea porque no se especificó archivo, no se encontró, o hubo error).")
